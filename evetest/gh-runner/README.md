...
cd .. # Run under evetest directory
...
- After the VM obtains a DHCP lease, `vm-create.sh` writes the VM's IP address
  to `runner/evetest-runner.env` as `export EVETEST_API_ADDRESS=<ip>`.
  Source this file in your terminal to point the `evetest` CLI at the runner.
  The file is deleted automatically by `vm-destroy.sh` and is listed in
  `.gitignore`.
