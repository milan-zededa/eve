...
func TestMyFeature(test *testing.T) {
    ...
    // For test output logging, use evetest.Logger().
    log := evetest.Logger()
    log.Infof("Device Port Config List: %s", dpcl.String())
...
evetest.RequireEdgeDevice{
    ...
    MinDiskSizeInMB:   16384,           // default: 16384
...
| `EVETEST_OUTPUT_FORMAT` | `go test` output format: `json` (machine-readable, for `gotestfmt`), `verbose` (human-readable `-v`), or unset (default compact output) | -- |
...
For each one:
1. Go to **Administration → Registries → New Endpoint**, enter the endpoint URL, and save.
2. Go to **Projects → New Project**, check **Proxy Cache**, select the endpoint.
3. Set the project **Access Level** to **Public** so containerd can pull without authentication.
...
Each value includes the `http://` scheme and the Harbor project path. Only the registries
you need to mirror must be set; the others pull directly from their origin.
