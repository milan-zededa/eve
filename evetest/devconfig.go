...
type ApplicationImageStorage interface {
	toProto(th *TestHarness, devName string, contentTreeUUID, datastoreUUID uuid.UUID,
...
func (container DockerContainer) toProto(th *TestHarness, devName string,
	...
	username, password, err := utils.GetDockerAuthPlain(
		log.NewEntry(log.StandardLogger()), dsConfig.Fqdn)
	if err != nil {
...
func (s3 AwsS3Bucket) toProto(th *TestHarness, devName string,
	contentTreeUUID, datastoreUUID uuid.UUID,
	appName string) (*eveconfig.ContentTree, *eveconfig.DatastoreConfig) {
	contentTree := &eveconfig.ContentTree{
		Uuid:         contentTreeUUID.String(),
...
func (azure AzureBlob) toProto(th *TestHarness, devName string,
	contentTreeUUID, datastoreUUID uuid.UUID,
	appName string) (*eveconfig.ContentTree, *eveconfig.DatastoreConfig) {
	contentTree := &eveconfig.ContentTree{
		Uuid:         contentTreeUUID.String(),
...
func (storage HTTPStorage) toProto(th *TestHarness, devName string,
	contentTreeUUID, datastoreUUID uuid.UUID,
	appName string) (*eveconfig.ContentTree, *eveconfig.DatastoreConfig) {
	contentTree := &eveconfig.ContentTree{
		Uuid:         contentTreeUUID.String(),
...
func (storage SFTPStorage) toProto(th *TestHarness, devName string,
	contentTreeUUID, datastoreUUID uuid.UUID,
	appName string) (*eveconfig.ContentTree, *eveconfig.DatastoreConfig) {
	contentTree := &eveconfig.ContentTree{
		Uuid:         contentTreeUUID.String(),
...
func (dc *EdgeDeviceConfig) AddNetwork(netConfig NetworkConfig) uuid.UUID {
	...
	if err != nil {
		dc.th.t.Fatalf("Failed to generate UUID for network: %v", err)
	}
...
func (dc *EdgeDeviceConfig) UpdateNetworkAdapter(config NetworkAdapterConfig) {
	...
	// Update SystemAdapter.
	var found bool
	for i, adapter := range dc.SystemAdapterList {
		if adapter.Name == config.LogicalLabel {
			dc.SystemAdapterList[i] = config.toSystemAdapterProto()
			found = true
			break
		}
	}
...
func (dc *EdgeDeviceConfig) setDefaultConfigProperties() {
	...
	defaultConfigItems := []*eveconfig.ConfigItem{
		...
		{Key: string(pillartypes.LocationAppInterval), Value: "10"},
...
func (dc *EdgeDeviceConfig) AddNetworkInstance(config NetworkInstanceConfig) uuid.UUID {
	...
	niUUID, err := uuid.NewV4()
	if err != nil {
		dc.th.t.Fatalf("Failed to generate UUID for network instance: %v", err)
	}
	protoConfig := config.toProto(dc.th, niUUID)
	for _, ni2 := range dc.NetworkInstances {
		if ni2.Displayname == protoConfig.Displayname {
			dc.th.t.Fatalf("Network instance with DisplayName %q already exists",
				protoConfig.Displayname)
		}
	}
...
func (dc *EdgeDeviceConfig) AddApplication(config ApplicationInstanceConfig) uuid.UUID {
	...
	if err != nil {
		dc.th.t.Fatalf("Failed to generate UUID for application: %v", err)
	}
	for _, app2 := range dc.Apps {
		if app2.Displayname == config.DisplayName {
			dc.th.t.Fatalf("Aplication with DisplayName %q already exists",
				config.DisplayName)
		}
	}
	volumeUUID, err := uuid.NewV4()
	if err != nil {
		dc.th.t.Fatalf("Failed to generate UUID for application volume: %v", err)
	}
	...
	contentTreeUUID, err := uuid.NewV4()
	if err != nil {
		dc.th.t.Fatalf(
			"Failed to generate UUID for application image content tree: %v", err)
	}
	datastoreUUID, err := uuid.NewV4()
	if err != nil {
		dc.th.t.Fatalf(
			"Failed to generate UUID for application image datastore: %v", err)
	}
	...
	contentTree, dsConfig := config.Image.toProto(dc.th, dc.DeviceName, contentTreeUUID,
		datastoreUUID, config.DisplayName)
