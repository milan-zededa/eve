...
func (p *QemuProvider) SetupDevice(
	...
	for i, tap := range dev.taps {
		dev.args = append(dev.args,
			...
			"-device", fmt.Sprintf(
				"virtio-net-pci,netdev=net%d,mac=%s", i, tap.guestMAC.String()),
		)
	}
	...
	dev.args = append(dev.args,
		"-chardev", fmt.Sprintf(
			"socket,id=char0,host=0.0.0.0,port=%d,server=on,wait=off,telnet=on,"+
...
func (p *QemuProvider) ensureXConnectNetwork(
	...
	// TODO: We also need to enable STP + LACP but group_fwd_mask does not allow that :((
