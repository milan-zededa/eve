...
//
// DNS server layout (all servers have no UpstreamServers, so only names in
// their StaticEntries are resolved; everything else returns NXDOMAIN):
//
		...
		DnsServers: []*api.DNSServer{
			{
				...
				// No UpstreamServers: names not in static entries return NXDOMAIN.
			},
			{
				Endpoint: &api.Endpoint{
