...
//
// The method discovers SSH endpoints for the application by inspecting the
// device configuration for port-forwarding ACL rules (port 22 mapped through
// a local network instance) and for interfaces connected to switch network
// instances (directly bridged, reachable on the app IP at port 22).
//
...
func (d *EdgeDevice) RunShellScriptInsideApp(appUUID uuid.UUID, auth AuthMethod,
	...
	for _, iface := range appConfig.GetInterfaces() {
		...
		for _, acl := range iface.GetAcls() {
			...
			// Find the external (device-side) port from "lport" match.
			for _, match := range acl.GetMatches() {
				if match.GetType() == "lport" {
			...
			// Resolve device IPs for the network instance port.
			deviceIPs := d.getDeviceIPsForNIPort(config, niConfig)
...
// getDeviceIPsForNIPort returns device IP addresses for the port(s) referenced
// by the given network instance config. The Port.Name field is matched against
// SystemAdapter.Name (logical label) or SystemAdapter.SharedLabels.
func (d *EdgeDevice) getDeviceIPsForNIPort(
	config *EdgeDeviceConfig, niConfig *eveconfig.NetworkInstanceConfig) []net.IP {
	portName := niConfig.GetPort().GetName()
	if portName == "" {
		return nil
	}
	var adapterLabels []string
	for _, sa := range config.GetSystemAdapterList() {
		if sa.GetName() == portName {
			// Exact match by logical label.
			adapterLabels = append(adapterLabels, sa.GetName())
			break
		}
		for _, label := range sa.GetSharedLabels() {
			if label == portName {
				adapterLabels = append(adapterLabels, sa.GetName())
				break
			}
	...
	for _, label := range adapterLabels {
		ips = append(ips, d.GetDeviceIPAddress(label)...)
	}
	return ips
}
...
func (d *EdgeDevice) WriteFile(fileName string, content []byte) {
	...
	err = d.th.scpToEVE(ctx, d.devName, tmpPath, fileName)
	if err != nil {
