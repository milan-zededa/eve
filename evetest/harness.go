...
func (th *TestHarness) maybeReuseDevice(dev deviceState, newReq RequireEdgeDevice) error {
	...
	case ResetDeviceConfig, ResetDeviceConfigAndReboot, ReonboardEdgeDevice:
		...
		cancel()
		if err != nil {
			th.t.Fatalf("Failed to apply the new (cleared) configuration "+
				"(version %d) for device %q: %v", configVer, dev.name, err)
		}
...
func (th *TestHarness) runScriptOnEVEOverSSH(ctx context.Context, devName string,
	...
	eveIP, err := th.getReachableEVEIP(ctx, devName)
	if err != nil {
		return err
	}

	sshArgs := append([]string{}, utils.EveSSHCommonArgs...)
...
// scpFromEVE copies a file from the device to a local path.
func (th *TestHarness) scpFromEVE(ctx context.Context,
	devName string, remotePath string, localPath string) error {
...
// getReachableEVEIP finds an SSH-reachable IP for the given device.
func (th *TestHarness) getReachableEVEIP(
	ctx context.Context, devName string) (string, error) {
	eveIPs, err := th.collectEVEIPs(ctx, devName)
	if err != nil {
		return "", err
	}
...
func (th *TestHarness) collectEVEIPs(
	...
	for _, port := range th.netModel.Ports {
		...
		var args []string
		args = append(args, utils.EveSSHCommonArgs...)
		args = append(args,
			"-i", "/root/.ssh/sdn_rsa",
...
func (th *TestHarness) nextConfigVersion(currentConfig *EdgeDeviceConfig) string {
	...
	if currentConfig != nil && currentConfig.GetId().GetVersion() != "" {
		...
		if err != nil {
			th.t.Fatalf("Failed to convert config version to integer: %w", err)
		}
	}
	configVer++
...
// Exactly one of infoMsg or reqEvent is non-nil.
...
func (th *TestHarness) handleDeviceInfoEvent(devName string, msg *eveinfo.ZInfoMsg) {
	...
	dev.state = zDeviceStateToEVEDeviceState(
