...
// OpenTCPProxy opens a generic TCP proxy to a given port on an EVE device.
func (th *TestHarness) OpenTCPProxy(
	ctx context.Context, req *api.OpenTCPProxyRequest) (*api.TCPProxyResponse, error) {
	devName, _, err := th.resolveEVEDeviceName(req.GetDeviceName())
	if err != nil {
		return nil, err
	}
	...
	if targetPort == 0 {
		return nil, fmt.Errorf("target_port must be specified")
	}
	...
	if err != nil {
		return nil, fmt.Errorf(
			"failed to start EVE device %q TCP proxy listener on %s: %w",
	...
	th.log.Infof("Started EVE device %q TCP proxy (target port %d) on %s:%d",
		devName, targetPort, th.dockerIntfIPv4.String(), addr.Port)

	th.wg.Add(1)
	go th.runTCPProxyForEVE(ln, devName, targetPort, interfaceName)
...
func (th *TestHarness) runTCPProxyForEVE(listener net.Listener,
	devName string, targetPort uint32, interfaceName string) {
	defer th.wg.Done()
	...
	for {
		...
		if err != nil {
			...
			if errors.Is(err, net.ErrClosed) ||
				...
				th.log.Debugf("EVE TCP proxy listener closed, stopping accept loop")
				return
			}

			th.log.Errorf("EVE TCP proxy accept error: %v", err)
			continue
		}
		th.log.Infof("Client connected to EVE TCP proxy (target port %d) from %s",
			targetPort, conn.RemoteAddr())
		...
		go func(c net.Conn) {
			...
			// Get a reachable EVE IP
			eveIP, err := th.getReachableEVEAddr(
				th.ctx, devName, targetPort, interfaceName)
			if err != nil {
				th.log.Errorf(
					"Unable to establish TCP connection to EVE device %q port %d: %v",
					devName, targetPort, err)
				return
			}

			// Dial to the reachable IP and target port
			dialer := net.Dialer{}
			addr := net.JoinHostPort(eveIP, targetPortStr)
			eveConn, err := dialer.DialContext(th.ctx, "tcp", addr)
			if err != nil {
				th.log.Errorf(
					"Failed to connect to EVE device %q endpoint %s:%d: %v",
					devName, eveIP, targetPort, err)
				return
			}
			defer eveConn.Close()
			th.log.Debugf("Connected to EVE device %q endpoint %s:%d",
				devName, eveIP, targetPort)
			...
			proxyLog := th.log.WithField("component", "eve-tcp-proxy")
			utils.RunPipeProxy(th.ctx, proxyLog, "EVE TCP", evePipe, clientPipe)
		}(conn)
	}
}
