...
// to all cluster devices in parallel. If waitUntilApplied is true, the function
// waits until each device has fetched and applied the new configuration.
...
func (ec *EdgeCluster) ApplyConfig(
	clusterConfig *EdgeClusterConfig, waitUntilApplied bool) {
	...
	RunParallel(len(nodes), func(i int) {
		...
		ec.devices[i].ApplyConfig(
			clusterConfig.GetDeviceConfig(node.DevName), waitUntilApplied)
	})
}
