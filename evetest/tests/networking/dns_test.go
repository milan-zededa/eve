...
func TestDNSFunctionality(test *testing.T) {
	...
	// dnsmasq must log eth3's server (cost=0, badDNS3IP) before any cost-1+
	// server for the same query. Logs may arrive with delay.
	...
	t.Eventually(func(g Gomega) {
		...
		if eth2Idx >= 0 {
			// eth2 may not appear if eth3 succeeded (NXDOMAIN stops the
			// retry chain); if it does, it must come after eth3.
			g.Expect(eth2Idx).To(BeNumerically(">", eth3Idx),
