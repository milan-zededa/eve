...
// exercises. Unlike a real deployment, the netboot server here is not
// SDN-hosted: dnsmasq's DHCP options 66/67 point clients directly at
// evetest's own HTTP/HTTPS/SFTP/TFTP image server (external to Evetest-SDN),
...
//     field set to evetest's own image server (same pattern
//     TestThreeNodesCluster uses to enable netboot on netmodels.SeparateClusterPort).
//
// Device configuration
// --------------------
...
func TestBootstrapWithNetworkBoot(test *testing.T) {
	...
	// Clone before mutating -- SingleEthWithDHCP is a shared global var, like
	// every other netmodels.X model (see e.g. failover_test.go's own
	// proto.Clone(netmodels.TwoMgmtPorts) uses).
