...
//     EVE over the network via iPXE (DeviceReusePolicy=CreateFromScratchWithNetworkBoot,
//     with the mgmt+app network's DHCP pointed at evetest's own image server)
//     instead of using a local live image, each device's own installer ISO
//     selected on the grub side by MAC address; requires CAPABILITY_NETBOOT
//     and is skipped on providers that lack it.
//
// Phases
// ------
...
func TestThreeNodesCluster(test *testing.T) {
	...
	// Clone before mutating -- SeparateClusterPort is a shared global var,
	// like every other netmodels.X model (see e.g. failover_test.go's own
