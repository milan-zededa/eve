	...
	for _, item := range appStatus.AppNetAdapterList {
		...
		for _, route := range niStatus.CurrentRoutes {
			...
			if route.GatewayApp != emptyUUID {
				...
				fmt.Printf("   - route dst: %s, port: %s\n",
					dstNet, route.OutputPort)
			}
			for _, routeStatus := range niStatus.IntendedRoutes {
				if routeStatus.DstNetwork.String() != dstNet {
					continue
				}
				if !routeStatus.RunningPortProbing {
					continue
				}
				for _, probeMetrics := range niMetrics.ProbeMetrics {
					...
					for _, intfMetrics := range probeMetrics.IntfProbeStats {
						upStatus := "DOWN"
						if intfMetrics.NexthopUP && intfMetrics.RemoteUP {
							upStatus = "UP"
						}
						fmt.Printf("       %s probe status: %s, "+
							"local success: %d, remote success: %d, remote latency: %d msec\n",
							intfMetrics.IntfName, upStatus, intfMetrics.NexthopUPCnt,
							intfMetrics.RemoteUPCnt, intfMetrics.LatencyToRemote)
					}
					break
				}
			}
