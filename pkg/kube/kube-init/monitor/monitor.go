...
type Monitor struct {
	...
	// containerd is the cached client used by reimportImages. Lazily
	// dialled on first use so we don't spend the boot budget on it,
	// and Closed by Stop so tests + shutdown don't leak the socket.
	containerdMu sync.Mutex
	containerd   *kubectlx.ContainerdClient
...
func (m *Monitor) Stop() {
	...
	m.containerdMu.Lock()
	if m.containerd != nil {
		_ = m.containerd.Close()
		m.containerd = nil
	}
	m.containerdMu.Unlock()
...
func (m *Monitor) reapplyNodeLabels(ctx context.Context) {
	...
	// disks annotation in a single API call. The verification below
	// reads the persisted node object directly rather than re-querying
	// by label selector, avoiding a write-read race.
	...
	labelPatch, err := kubectlx.BuildMergeLabelPatch(
		...
		map[string]string{
			"node.longhorn.io/default-disks-config": string(disks),
		},
	)
	if err != nil {
...
// dialling it on first use. Callers must NOT Close it — Monitor.Stop
// owns the lifecycle.
func (m *Monitor) getContainerd(ctx context.Context) (*kubectlx.ContainerdClient, error) {
	m.containerdMu.Lock()
	defer m.containerdMu.Unlock()
	if m.containerd != nil {
		return m.containerd, nil
	}
...
func (m *Monitor) assignMultusNodeIP() error {
	_, ip, err := components.DefaultInterfaceIPv4()
	if err != nil {
		return err
	}
