...
const (
	...
	EvDeployDone                       // components.DeployAll finished
	EvSnapshotDone                     // /var/lib snapshot + markers written
	EvK3sExited                        // supervisor.Done fired
	EvStopDone                         // supervisor.Stop finished, ports freed
	EvHooksDone                        // supervisor.RunHooks finished
	EvHooksFailed                      // a required hook failed; k3s must not start
...
type Event struct {
	...
	// HookResults carries what RunHooks observed, on EvHooksDone and
	// EvHooksFailed.
...
const (
	...
	StateSnapshot                       // quiesce k3s and snapshot /var/lib
...
type daemon struct {
	...
	// hookResults is the last pre-restart hook run, reported by the
	// control socket.
	hookResults []k3s.HookResult
	...
	// statusPublishReady gates KubeInitStatus publication until the
	// publisher exists. Transitions happen before pubsub is wired, and
	// a failed publish there is expected, not worth logging.
	statusPublishReady bool
...
func (d *daemon) handleDeploying(ctx context.Context, ev Event) {
	...
	case EvDeployDone:
		...
		// Phase stays PhaseFirstBoot until the snapshot is on disk;
		// SNAPSHOT flips it so the k3s restart it performs routes
		// straight to RUNNING instead of re-entering DEPLOYING.
		d.lastError = nil
		d.transition(ctx, StateSnapshot, "deploy-done")
...
// handleSnapshot drives the quiesce-snapshot-restart sequence that ends
// first-boot initialization.
//
// The snapshot is the rollback point a later cluster→single conversion
// restores, and /var/lib holds k3s's datastore. Copying that while k3s
// is writing to it risks a torn database, so k3s is stopped first — the
// same order the shell implementation used. The cost is one k3s restart
...
func (d *daemon) handleSnapshot(ctx context.Context, ev Event) {
	...
	case EvError:
		...
		// conversion has nothing to roll back to. Retrying re-enters
		// this state, whose Stop is a no-op on already-stopped k3s.
		d.lastError = ev.Err
...
// this state needs is that nothing is writing to the datastore, and the
// process has exited by the time Stop reports that. The ports linger
// because a just-finished deploy leaves many established API
// connections whose sockets sit in TIME_WAIT past Stop's budget —
// failing on it would spend an error and a retry cycle on every first
...
func (d *daemon) enterSnapshot(ctx context.Context) {
	...
	d.startAsync(ctx, func(asyncCtx context.Context) error {
		// Wait, bounded, for BestEffort retries to settle before
		// capturing the tree. Reaching this state means every component
		// either converged or was best-effort'd past, so work can still
		// be in flight, and a component still reconciling would be
		// snapshotted half-deployed — which is what a later
		// cluster→single rollback would then restore. Never wait
		// indefinitely: a genuinely wedged component must not block the
		// snapshot.
		...
		return d.workSnapshot(asyncCtx)
	}, EvSnapshotDone)
}
...
func (d *daemon) workSnapshot(_ context.Context) error {
	...
	if err := state.SaveVarLib(); err != nil {
		return fmt.Errorf("save /var/lib snapshot (required to roll back a "+
			"future cluster→single conversion): %w", err)
	}

	// Written after the snapshot, so the restored tree looks
	// "components installed, not yet initialized" and the post-restore
	// boot re-runs an idempotent deploy. Same order as the shell.
...
func (d *daemon) handleClusterTransition(ctx context.Context, ev Event) {
	...
	case EvTransitionDone:
		...
		// A join is its own episode, so the furthest-stage mark starts
		// over here. It is a high-water mark, and first boot has
		// already climbed it to SNAPSHOT by the time a transition runs
		// — leaving it set means every stage of the join that follows
		// (STARTING_K3S, IMPORTING, WAIT_K3S_READY) ranks below the
		// mark, records no progress, and the watchdog reboots a
		// perfectly healthy node at the stall limit. That is what
		// happened to edge-dev2 on 2026-08-02, twice.
...
func (d *daemon) handleRunningHooks(ctx context.Context, ev Event) {
	...
	case EvHooksFailed:
		// A hook under hooks.d/required/ declares that k3s must not run
		// without whatever it installs — an RT profile change whose
		// cache repartition failed would otherwise start k3s against
		// stale configuration. Back off and retry the whole restart,
		// which re-runs the hook. Hooks outside that directory keep the
		// log-and-continue behaviour handled by EvError below.
		d.hookResults = ev.HookResults
		var failed []string
		for _, r := range ev.HookResults {
			if r.Required && r.Err != nil {
				failed = append(failed, fmt.Sprintf("%s (exit %d)", r.Name, r.ExitCode))
			}
		}
		d.lastError = fmt.Errorf("required hook(s) failed: %s",
			strings.Join(failed, ", "))
		log.Printf("RUNNING_HOOKS: %v — NOT starting k3s", d.lastError)
		d.transition(ctx, StateBackoff, "hooks-failed/required")
	...
	case EvError:
		...
		// state until manual intervention. Hooks that must succeed
		// opt in via hooks.d/required/ and surface as EvHooksFailed.
		d.lastError = ev.Err
...
func (d *daemon) transition(ctx context.Context, newState State, reason string) {
	...
	// Publish before running the entry action: the entry action can
	// block for minutes (image import, readiness waits), and the whole
	// point is that an observer sees which state the node is stuck in
	// while it is stuck, not afterwards.
	d.publishStatus()
	d.enterStateFn(ctx)
}
...
// workDeploy deploys the cluster components. The /var/lib snapshot and
// the first-boot markers follow in StateSnapshot, which stops k3s first
// so the snapshot does not copy a datastore mid-write.
func (d *daemon) workDeploy(workCtx context.Context) error {
	return components.DeployAll(workCtx, d.rootCtx, d.signals,
		d.deviceName, d.installKubevirt)
}
...
func (d *daemon) enterRunning(ctx context.Context) {
	...
	// Start the CNI DHCP daemon eagerly. On PhaseSteady the DeployAll
	// graph is skipped, so without the eager start the daemon only
	// comes up on the first health tick — potentially 15+ seconds
	// after RUNNING — which delays pod admission for EVE apps using
	// multus dhcp-IPAM. StartDHCPDaemon is idempotent.
...
// runHooks runs the pre-restart hooks off the FSM goroutine and reports
// EvHooksFailed when one that must succeed did not, so the caller
// declines to start k3s against configuration the hook was supposed to
// install. Results are retained for the control socket.
func (d *daemon) runHooks(ctx context.Context) {
	workCtx, cancel := context.WithCancel(ctx)
	d.cancelWork = cancel

	go func() {
		var results []k3s.HookResult
		if d.supervisor != nil {
			results = d.supervisor.RunHooks()
		}
		if workCtx.Err() != nil {
			return
		}
		ev := Event{Type: EvHooksDone}
		if k3s.RequiredHookFailed(results) {
			ev = Event{Type: EvHooksFailed}
		}
		d.eventCh <- Event{Type: ev.Type, HookResults: results}
	}()
}
...
func (d *daemon) statusString() string {
	...
	if s := hookResultsString(d.hookResults); s != "" {
		parts = append(parts, s)
	}
	if s := signalsString(d.signals); s != "" {
		parts = append(parts, s)
	}
	return "OK: " + strings.Join(parts, " ")
}

// graphReport renders the deploy graph with each signal edge annotated
// by whether its condition holds, plus the set of components still
// blocked and what they are blocked on. bus may be nil, in which case
// every condition reads as unmet.
func graphReport(edges []deploy.Edge, bus *deploy.Bus) string {
	if len(edges) == 0 {
		return "OK: graph has no dependency edges\n"
	}
	var met map[deploy.Signal]bool
	if bus != nil {
		met = bus.Snapshot()
	}

	// Align the arrow column so a dozen edges stay scannable.
	fromW, toW := 0, 0
	for _, e := range edges {
		fromW = max(fromW, len(e.From))
		toW = max(toW, len(e.To))
	}

	var b strings.Builder
	b.WriteString("OK: deploy graph\n")
	blockedOn := make(map[string][]string)
	for _, e := range edges {
		fmt.Fprintf(&b, "  %-*s → %-*s  ", fromW, e.From, toW, e.To)
		if e.Rule != "signal" {
			fmt.Fprintf(&b, "%s\n", e.Rule)
			continue
		}
		...
		if met[e.Signal] {
			...
			blockedOn[e.To] = append(blockedOn[e.To], string(e.Signal))
		}
		fmt.Fprintf(&b, "signal %s [%s]\n", e.Signal, state)
	}

	// Signals the graph declares, so an unmet condition nobody is
	// waiting on yet is still visible.
	declared := make(map[deploy.Signal]bool)
	for _, e := range edges {
		if e.Rule == "signal" {
			declared[e.Signal] = true
		}
	}
	for s := range met {
		declared[s] = true
	...
	for _, n := range names {
		state := "waiting"
		if met[deploy.Signal(n)] {
			state = "met"
		}
		fmt.Fprintf(&b, " %s=%s", n, state)
	}
	b.WriteString("\n")

	if len(blockedOn) == 0 {
		b.WriteString("blocked: none\n")
		return b.String()
	}
	gated := make([]string, 0, len(blockedOn))
	for name := range blockedOn {
		gated = append(gated, name)
	}
	sort.Strings(gated)
	b.WriteString("blocked:")
	for _, name := range gated {
		sort.Strings(blockedOn[name])
		fmt.Fprintf(&b, " %s(%s)", name, strings.Join(blockedOn[name], ","))
	}
	b.WriteString("\n")
	return b.String()
}

// hookResultsString renders the last pre-restart hook run as
// hooks=name:ok,other:exit3, prefixing a required hook with '!' so an
// operator can tell which failure blocked the restart.
func hookResultsString(rs []k3s.HookResult) string {
	if len(rs) == 0 {
		return ""
	}
	...
	for _, r := range rs {
		...
		if r.Err == nil {
			items = append(items, name+":ok")
			continue
		}
		items = append(items, fmt.Sprintf("%s:exit%d", name, r.ExitCode))
	}
	return "hooks=" + strings.Join(items, ",")
}

// signalsString renders the latched readiness signals, sorted so the
// status line is stable across calls.
func signalsString(bus *deploy.Bus) string {
	if bus == nil {
		return ""
	}
	snap := bus.Snapshot()
	...
	sort.Strings(names)
	return "signals=" + strings.Join(names, ",")
}
