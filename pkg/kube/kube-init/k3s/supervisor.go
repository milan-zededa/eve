...
// requiredHooksSubdir holds hooks that must succeed for the restart to
// proceed. Opting in by placing a hook here is what distinguishes "k3s
// must not start without this" from the default, where an
// operator-defined hook failing must not trap the device.
const requiredHooksSubdir = "required"

// HookResult records one hook invocation so the FSM can decide whether
// to proceed and the control socket can report what ran.
...
// RequiredHookFailed reports whether any hook that must succeed did not.
func RequiredHookFailed(rs []HookResult) bool {
	for _, r := range rs {
...
// RunHooks executes the hooks in requiredHooksSubdir, then every
// executable directly in the hooks directory, each in lexical order.
//
// A failure among the latter is logged and the sequence continues: those
// are operator-defined cleanups and refusing to restart because one has
// a bug would trap a misconfigured device. A failure among the former is
// reported to the caller, which declines to start k3s rather than run it
// against configuration a required hook was supposed to install.
func (s *Supervisor) RunHooks() []HookResult {
	required := s.runHookDir(filepath.Join(s.hooksDir, requiredHooksSubdir), true)
	return append(required, s.runHookDir(s.hooksDir, false)...)
}

// runHookDir runs the executables directly inside dir. A missing dir is
// not an error — neither hooks directory is required to exist.
func (s *Supervisor) runHookDir(dir string, required bool) []HookResult {
	entries, err := os.ReadDir(dir)
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			log.Printf("failed to read hooks dir %s: %v", dir, err)
		}
		return nil
	}
	...
	for _, entry := range entries {
		...
		log.Printf("running hook: %s (required=%v)", hookPath, required)
		...
		if runErr != nil {
			verb := "continuing"
			if required {
				verb = "restart will not proceed"
			}
			log.Printf("hook %s failed after %s: %v (%s)",
				hookPath, res.Duration.Round(time.Millisecond), runErr, verb)
		}
...
// "device not present" is the common case.
