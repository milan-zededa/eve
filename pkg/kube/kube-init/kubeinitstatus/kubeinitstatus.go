...
type KubeInitStatus struct {
	...
	// The fields below describe what the daemon is doing right now.
	// They exist because AllComponentsInitialized on its own is not
	// enough to tell a healthy node from a stuck one: on 2026-08-02 a
	// node reported it true for twenty minutes while it was unable to
	// either join its cluster or convert back, and answering "what is
	// this node doing" needed an ssh session and two log files. Every
	// field here is derived from state the daemon already holds.

	// State is the FSM state (INIT, WAIT_K3S_READY, RUNNING, …).
	State string

	// Phase is first-boot / steady / recycle.
	Phase string

	// Membership is where the node sits in the single↔cluster
	// lifecycle: Single, JoinPending, Joining, Member, Leaving or
	// Restoring. Derived, not stored — see the deriving function for
	// the precedence rules.
	Membership string

	// ClusterID is the cluster this node belongs to or is joining,
	// empty when it is standalone.
	ClusterID string

	// Role is Bootstrap, Server or Worker; empty when standalone.
	Role string

	// TransitionStep names the single→cluster step in flight, empty
	// otherwise. This is the field that would have said "multus-reset"
	// while a node sat there for 38 seconds and then bricked itself.
	TransitionStep string

	// JoinWatchdogActive reports whether the stuck-join watchdog is
	// armed, so a reboot it causes is attributable without log
	// archaeology.
	JoinWatchdogActive bool

	// LastError is the most recent error the FSM recorded, empty once
	// a state completes cleanly.
	LastError string

	// UpdatedAt timestamps this snapshot, so a consumer can tell a
	// stale publication from a live one.
	UpdatedAt time.Time
}
