...
func (c *Client) getModemStatus(modemObj dbus.BusObject) (
	...
	for i, simPath := range simSlots {
		...
		if simPath.IsValid() && len(simPath) > 1 {
			...
			switch simType {
			case SIMTypeESIM:
				// eSIM is not support by EVE (or even by ModemManager) for connection
...
func (c *Client) getModemPhysAddrs(modemObj dbus.BusObject) (
	...
	if err != nil {
		...
		devPathSymlink = filepath.Join("/sys/class/wwan", primaryPort, "device")
		devPath, err = filepath.EvalSymlinks(devPathSymlink)
		if err != nil {
...
func (c *Client) preferPhysSimOverESim(modemObj dbus.BusObject) error {
	...
	c.log.Noticef("HEY! auto-switching SIM slot to %d", firstPhysSlot)
