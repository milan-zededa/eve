...
mbim() {
  ...
  local OUTPUT
  local RV
  OUTPUT="$(timeout -s INT -k 5 "$CMD_TIMEOUT" mbimcli -p -d "/dev/$CDC_DEV" "$@")"
  RV=$?
  if [ "$VERBOSE" = "true" ]; then
...
mbim_get_active_sim_slot() {
  ...
  # is inactivate (instead of "state-off-empty" which would be correct).
  local EMPTY_SLOT=""
  local SLOT=""
...
mbim_log_registration_debug_info() {
  if [ "$VERBOSE" = "true" ]; then
    ...
    mbim --query-preferred-providers 2>&1 >/dev/null
    mbim --query-signal-state 2>&1 >/dev/null
