...
# Used to encrypt user password between NIM and this microservice.
PASSPHRASE="$(cat /proc/sys/kernel/random/boot_id)"
...
log_debug() {
  ...
  echo "$(date "+%T") $MSG" | join_lines_with_escaped_newline >> "/tmp/wwan.log" # TODO: remove
}

log_error() {
  local MSG="${@}"
  echo "Error: $MSG" | join_lines_with_escaped_newline > "$LOG_PIPE"
  echo "$(date "+%T") Error: $MSG" | join_lines_with_escaped_newline >> "/tmp/wwan.log" # TODO: remove
  ...
  while read -r NETWORK; do
    ...
    if ! lookup_modem "${IFACE}" "${USB_ADDR}" "${PCI_ADDR}" 2>/tmp/wwan.stderr; then
      ...
      NETWORK_STATUS="$(json_struct \
        ...
        "$(json_str_attr config-error   "$CONFIG_ERROR")")"
      STATUS="${STATUS}${NETWORK_STATUS}\n"
      if ! cat "${STATUS_PATH}" | grep -q "$CONFIG_ERROR"; then
        STATUS_OUTDATED=y
      fi
      continue
    fi
    ...
    if [ "$RADIO_SILENCE" != "true" ]; then
      ...
      if [ "$EVENT" = "CONFIG-CHANGE" ] || ! check_connectivity "$EVENT"; then
        ...
        if [ -n "$USERNAME" ]; then
          ...
          PASSWORD="$(echo "$ENC_PASSWORD" |\
            openssl enc -base64 -d -aes-256-cbc -pass "pass:$PASSPHRASE" -pbkdf2)"
          log_debug "[$CDC_DEV] User password is: ${PASSWORD}" # TODO: remove
          RV=$?
          if [ $RV -ne 0 ]; then
            CONFIG_ERROR="failed to decrypt user password (rv=$RV)"
            NETWORK_STATUS="$(json_struct \
                    "$(json_str_attr logical-label  "$LOGICAL_LABEL")" \
                    "$(json_attr     physical-addrs "$ADDRS")" \
                    "$(json_str_attr config-error   "$CONFIG_ERROR")")"
  ...
  if [ "$EVENT" = "METRICS" ]; then
    ...
    json_struct \
      ...
      "$(json_str_attr config-checksum "$CHECKSUM")" \
        | tee /tmp/wwan-status.json | jq > "${STATUS_PATH}.tmp"
