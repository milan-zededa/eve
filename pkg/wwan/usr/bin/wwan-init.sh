...
probe_connectivity() {
  if [ "$PROBE_DISABLED" = "true" ]; then
    ...
    echo "Probe is disabled"
    return 0
  fi
  if [ -n "$PROBE_ADDR" ]; then
    # User-configured ICMP probe address.
    echo "Running ICMP probe towards: $PROBE_ADDR"
  ...
  if ${PROTOCOL}_get_ip_settings; then
    ...
    while read -r PROXY; do
      ...
      echo "Have proxy $SERVER $PORT"
      if echo "$SERVER" | grep -q "$IPV4_REGEXP"; then
        echo "Trying TCP handshake with proxy $SERVER $PORT"
        if nc -w 5 -s "$IP" -z -n "$SERVER" "$PORT" >/dev/null 2>&1; then
          echo "TCP handshake with proxy $SERVER $PORT OK"
          return 0
        fi
      fi
    done <<__EOT__
...
__EOT__
  ...
  echo "Running ICMP probe towards: $DEFAULT_PROBE_ADDR"
