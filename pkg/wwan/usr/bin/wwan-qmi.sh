...
qmi() {
  ...
  # TODO: somehow capture stderr and log it with log_error?
  local OUTPUT
  local RV
  OUTPUT="$(timeout -s INT -k 5 "$CMD_TIMEOUT" qmicli -p -d "/dev/$CDC_DEV" "$@")"
  RV=$?
  if [ "$VERBOSE" = "true" ]; then
...
qmi_log_registration_debug_info() {
  if [ "$VERBOSE" = "true" ]; then
    ...
    qmi --nas-get-system-selection-preference 2>&1 >/dev/null
    qmi --nas-get-preferred-networks 2>&1 >/dev/null
    qmi --nas-get-cell-location-info 2>&1 >/dev/null
    qmi --nas-get-signal-strength 2>&1 >/dev/null
    qmi --nas-get-rf-band-info 2>&1 >/dev/null
  fi
}
