...
__EOT__
    if [ -n "$verbose" ] && [ -x "$(command -v picocom)" ]; then
        ...
        if [ -n "$AT_PORT" ]; then
            ...
            send_hayes_command "+++" "${AT_PORT}"
            # Return modem identification.
            # Use sed to remove lines containing only whitespace.
            OUTPUT="$(send_hayes_command "ATI" "${AT_PORT}" | sed '/^\s*$/d')"
            if echo "$OUTPUT" | grep -qv "ERROR"; then
                # Remove control commands.
                OUTPUT="$(echo "$OUTPUT" | sed '/^\s*ATI\s*$/d; /^\s*OK\s*$/d')"
                # Join multiple lines by semi-colon.
                OUTPUT="$(echo "$OUTPUT" | sed -z 's/\r\n/; /g;s/; $//')"
                cat <<__EOT__
      "description": "$OUTPUT",
__EOT__
            fi
