...
func (m *DpcManager) getBondStatus(
	...
	// Active slave: resolve ifindex to logical label.
	if bondStatus.ActiveSlaveIfIndex > 0 {
		slaveAttrs, err := m.NetworkMonitor.GetInterfaceAttrs(bondStatus.ActiveSlaveIfIndex)
		if err == nil {
			if slavePort := dpc.LookupPortByIfName(slaveAttrs.IfName); slavePort != nil {
				portStatus.BondStatus.ActiveSlave = slavePort.Logicallabel
			}
		}
	}
	...
	// Per-slave status (published for all bond modes).
	for _, slave := range bondStatus.Slaves {
		slaveAttrs, err := m.NetworkMonitor.GetInterfaceAttrs(slave.IfIndex)
		if err != nil {
			continue
		}
		slaveLL := slaveAttrs.IfName
		if slavePort := dpc.LookupPortByIfName(slaveAttrs.IfName); slavePort != nil {
			slaveLL = slavePort.Logicallabel
		}
		slaveStatus := types.BondSlaveStatus{
			Logicallabel: slaveLL,
			MIIUp:        slave.MIIUp,
		}
		if isLACP {
			slaveStatus.LACP = &types.BondSlaveLACPStatus{
				AggregatorID:      slave.AggregatorID,
				ActorChurnState:   slave.ActorChurnState,
				PartnerChurnState: slave.PartnerChurnState,
			}
		}
		portStatus.BondStatus.Slaves = append(portStatus.BondStatus.Slaves, slaveStatus)
	}
}

// logBondActiveSlaveChange logs a bond active slave change using the already
// updated DeviceNetworkStatus to resolve labels.
func (m *DpcManager) logBondActiveSlaveChange(bondIfIndex int) {
	...
	if port != nil {
		...
		if port.BondStatus.ActiveSlave != "" {
			activeSlave = port.BondStatus.ActiveSlave
		}
	}
	m.Log.Noticef("Bond %s active slave changed to %s", bondLabel, activeSlave)
}
