...
type RequestOptions struct {
	...
	// DisableDNSCoalescing gives this request's DNS lookups (on the
	// non-traced path; tracing already does its own isolated resolution)
	// their own private resolver instead of sharing net.DefaultResolver's
	// singleflight group with the rest of the process. See
	// DialerWithSrcIP.DisableDNSCoalescing for why this matters for DPC
	// connectivity verification specifically, and why it is not the
	// default for regular traffic.
	DisableDNSCoalescing bool
}
...
type DialerWithSrcIP struct {
	...
	// DisableDNSCoalescing, when set, gives this dial its own private
	// *net.Resolver instead of the implicit net.DefaultResolver.
	// net.DefaultResolver is one global singleton shared by the whole
	// zedbox process (every agent, every port), and Go's resolver coalesces
	// concurrent lookups for the same hostname into a single in-flight
	// query (singleflight), keyed only by hostname -- with no awareness of
	// which port/DPC is asking. That sharing is a useful, deliberate
	// optimization for regular traffic (fewer redundant DNS queries), but
	// it is wrong for DPC connectivity verification (VerifyAllIntf): during
	// a DPC switch, a lookup for the controller hostname still in flight
	// from the *previous* (working) DPC can get coalesced with a fresh
	// lookup issued under a *new* DPC whose DNS config is actually broken
	// (e.g. a user typo in a static DNS server), handing the new lookup a
	// stale, unrelated success. A private Resolver has its own singleflight
	// group, so it can never be coalesced with lookups from any other
	// call, port, or agent -- set this only where that isolation matters.
	DisableDNSCoalescing bool
}
...
func (c *Client) VerifyAllIntf(ctx context.Context,
	...
	for _, intf := range intfs {
		...
		sendOnIntfOpts.UseOnboard = false
		// DPC connectivity verification must not let a lookup from one
		// port/DPC generation get coalesced with an unrelated one still in
		// flight for another -- see DialerWithSrcIP.DisableDNSCoalescing.
		sendOnIntfOpts.DisableDNSCoalescing = true
	...
	for i, addr := range portStatus.AddrInfoList {
		...
		if !opts.WithNetTracing {
			dialer := &DialerWithSrcIP{
				...
				DisableDNSCoalescing: opts.DisableDNSCoalescing,
			}
