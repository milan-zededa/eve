			...
			case netmonitor.AddrChange:
				...
				if brForNI != nil {
					if brForNI.config.Type == types.NetworkInstanceTypeSwitch {
						...
						brIPChanged := r.updateCurrentNIBridgeIP(brForNI.config.UUID)
						if brIPChanged {
...
func (r *LinuxNIReconciler) updateNIStatus(
	...
	inProgress, failedItems := r.getSubgraphState(intSG, currSG)
...
func (r *LinuxNIReconciler) getSubgraphState(intSG, currSG dg.GraphR) (
	...
	if intSG == nil {
		emptyCurrSG := currSG.Items(true).Next() == false
		return !emptyCurrSG, failedItems
	}
	diff := currSG.DiffItems(intSG)
	for _, itemRef := range diff {
		// Ignore if extra item is external.
		_, _, _, shouldExist := intSG.Item(itemRef)
		item, _, _, exists := currSG.Item(itemRef)
		if exists && !shouldExist && item.External() {
			continue
		}
		// Also ignore extra routes added by kernel.
		route, isRoute := item.(linux.Route)
		if exists && !shouldExist && isRoute {
			ipv6 := route.Dst != nil && route.Dst.IP.To4() == nil
			if ipv6 && route.Dst.IP.IsLinkLocalUnicast() {
				continue
			}
		}
		inProgress = true
