...
func (info *UuidToNum) New(key objtonum.ObjKey) objtonum.ObjNumContainer {
	return &UuidToNum{
		UuidToNumKey: key.(UuidToNumKey),
...
type AppInterfaceKey struct {
	...
	AppID uuid.UUID
	// IfIdx : interface index among interfaces of this app (first has index 0, next 1, etc.)
	IfIdx uint32
}
...
// AppInterfaceToNum is used to store number assigned to an application
// interface. The number is unique only within the network instance where the interface
...
// New is used by objtonum.ObjNumPublisher.
func (info *AppInterfaceToNum) New(key objtonum.ObjKey) objtonum.ObjNumContainer {
	return &AppInterfaceToNum{
		AppInterfaceKey: key.(AppInterfaceKey),
		LastUseTime:     time.Now(),
		CreateTime:      time.Now(),
	}
}
