...
// ActivationPending returns true if the network instance is not activated
// yet it should be.
// This could happen if there was some failure during activation or if uplink
// interface is missing, etc.
func (status NetworkInstanceStatus) ActivationPending() bool {
	return status.NetworkInstanceConfig.Activate && !status.Activated
}
