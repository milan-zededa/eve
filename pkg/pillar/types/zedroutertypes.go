...
const (
	// Not an L2 link (used for physical network adapters).
...
// MII link monitoring parameters (see devmodel.proto for description).
...
// ARP-based link monitoring parameters (see devmodel.proto for description).
...
type NetworkPortConfig struct {
	...
	IsEndpoint  bool  // Referenced by SystemAdapter vs. used only as a VLAN parent or a bond slave
...
// Check if an interface name is a port owned by nim/zedrouter
...
// Check if an interface name belongs to a port with SystemAdapter attached.
func IsEndpoint(globalStatus DeviceNetworkStatus, ifname string) bool {
	for _, us := range globalStatus.Ports {
		if us.IfName != ifname {
			continue
		}
		return us.IsEndpoint
	}
	return false
}
