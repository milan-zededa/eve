...
type BondStatus struct {
	...
	Mode BondMode
	// ActiveSlave is the logical label of the currently active slave interface.
	...
	LACP BondLACPStatus
	// Slaves contains per-slave runtime status.
	Slaves []BondSlaveStatus
}

// BondSlaveStatus : per-slave runtime status within a bond.
type BondSlaveStatus struct {
	// Logicallabel of the slave port.
	Logicallabel string
	// MIIUp indicates whether the slave's MII link is up.
	MIIUp bool
	// LACP contains LACP-specific status (802.3ad only).
	LACP *BondSlaveLACPStatus
}
...
// BondSlaveLACPStatus : per-slave LACP status.
type BondSlaveLACPStatus struct {
	// AggregatorID to which this slave is assigned.
	AggregatorID uint16
	// ActorChurnState for this slave.
	ActorChurnState BondLACPChurnState
	// PartnerChurnState for this slave.
...
func (s BondStatus) Equal(s2 BondStatus) bool {
	return s.Mode == s2.Mode &&
		s.ActiveSlave == s2.ActiveSlave &&
		s.MIIMonitor == s2.MIIMonitor &&
		s.ARPMonitor.Equal(s2.ARPMonitor) &&
		s.LACP.Equal(s2.LACP) &&
		generics.EqualSetsFn(s.Slaves, s2.Slaves, func(a, b BondSlaveStatus) bool {
			return a.Equal(b)
		})
}

// Equal compares two BondSlaveStatus values.
func (s BondSlaveStatus) Equal(s2 BondSlaveStatus) bool {
...
type BondMetrics struct {
	...
	LogicalLabel string
	// Slaves contains per-slave metrics.
	Slaves []BondSlaveMetrics
}

// BondSlaveMetrics contains metrics for a single slave within a bond.
type BondSlaveMetrics struct {
	// LogicalLabel of the slave port.
	LogicalLabel string
	// IfName of the slave interface.
	IfName string
	...
	LACP *BondSlaveLACPMetrics
}

// BondSlaveLACPMetrics contains LACP (802.3ad) counters for a bond slave.
type BondSlaveLACPMetrics struct {
