...
func SendOnIntf(workContext context.Context, ctx *ZedCloudContext, destURL string,
	...
	for retryCount := 0; retryCount < addrCount; retryCount += 1 {
		...
		if err != nil {
			if withNetTracing {
				// Wait just a little bit to capture all the packets of the failed request.
				// This overhead is introduced only when the request is traced, which
		...
		if withNetTracing {
			// Wait just a little bit to capture all the packets of the traced connection.
			// This overhead is introduced only when the request is traced, which
			// is not very often (e.g. once per day).
			time.Sleep(250 * time.Millisecond)
			netTrace, pcaps, err := tracedClient.GetTrace(tracedReqDescr)
