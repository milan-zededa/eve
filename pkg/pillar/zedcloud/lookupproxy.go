...
func LookupProxy(log *base.LogObject, port types.NetworkPortStatus,
	rawUrl string) (*url.URL, error) {

	log.Tracef("LookupProxy: Looking for proxy config on port %s", port.Logicallabel)
	...
	for _, proxy := range proxyConfig.Proxies {
		...
		case types.NPT_HTTP:
			...
			log.Tracef("LookupProxy: Adding HTTP proxy %s for port %s",
				config.HTTPProxy, port.Logicallabel)
		case types.NPT_HTTPS:
			...
			log.Tracef("LookupProxy: Adding HTTPS proxy %s for port %s",
				config.HTTPSProxy, port.Logicallabel)
		default:
...
// PortLookupProxyCfg - check if the network port has proxy configured.
func PortLookupProxyCfg(log *base.LogObject, port types.NetworkPortStatus, downloadURL string,
	...
	tmpURL := passURL
	tmpURL.Scheme = "https"
	proxyURL, err := LookupProxy(log, port, tmpURL.String())
