...
func TestAllocator(test *testing.T) {
	...
	key = TestObjKey{ObjName: "my-obj256", ObjType: "obj-type1"}
...
func TestAllocatorGC(test *testing.T) {
	...
	err = allocator.FreeMultiple(func(key objtonum.ObjKey) bool {
...
func TestAllocStrategies(test *testing.T) {
	...
	key = TestObjKey{ObjName: "my-obj4", ObjType: "obj-type1"}
	num, err = allocator.GetOrAllocate(key, objtonum.HighestFree)
	t.Expect(err).ToNot(HaveOccurred())
	t.Expect(num).To(Equal(254))
	key = TestObjKey{ObjName: "my-obj5", ObjType: "obj-type1"}
...
func TestAllocatorRestart(test *testing.T) {
	...
	publisher, err := objtonum.NewObjNumPublisher(
		ps, "test-agent", true, &TestObjNumContainer{})
	t.Expect(err).ToNot(HaveOccurred())
	...
	t.Expect(err).ToNot(HaveOccurred())
	t.Expect(num).To(Equal(1))
	key = TestObjKey{ObjName: "my-obj2", ObjType: "obj-type2"}
	_, err = allocator.GetOrAllocate(key)
	t.Expect(err).To(HaveOccurred())
...
func TestReusedOldestReserved(test *testing.T) {
	...
	key := TestObjKey{ObjName: "my-obj42", ObjType: "obj-type1"}
	err = allocator.Free(key, true)
	t.Expect(err).ToNot(HaveOccurred())
	key = TestObjKey{ObjName: "my-obj13", ObjType: "obj-type1"}
	err = allocator.Free(key, true)
	t.Expect(err).ToNot(HaveOccurred())
	key = TestObjKey{ObjName: "my-obj109", ObjType: "obj-type1"}
	err = allocator.Free(key, true)
	t.Expect(err).ToNot(HaveOccurred())
	key = TestObjKey{ObjName: "my-obj201", ObjType: "obj-type1"}
	err = allocator.Free(key, true)
	t.Expect(err).ToNot(HaveOccurred())
	...
	key = TestObjKey{ObjName: "my-obj257", ObjType: "obj-type1"}
	num, err = allocator.GetOrAllocate(key)
	t.Expect(err).ToNot(HaveOccurred())
	t.Expect(num).To(Equal(13)) // second oldest reserved
	key = TestObjKey{ObjName: "my-obj258", ObjType: "obj-type1"}
	num, err = allocator.GetOrAllocate(key)
	t.Expect(err).ToNot(HaveOccurred())
	t.Expect(num).To(Equal(109))
	key = TestObjKey{ObjName: "my-obj259", ObjType: "obj-type1"}
	num, err = allocator.GetOrAllocate(key)
	t.Expect(err).ToNot(HaveOccurred())
	t.Expect(num).To(Equal(201))

	// Nothing reserved to steal anymore.
	key = TestObjKey{ObjName: "my-obj260", ObjType: "obj-type1"}
	num, err = allocator.GetOrAllocate(key)
	t.Expect(err).To(HaveOccurred())
}
