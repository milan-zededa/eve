...
func NewObjNumPublisher(ps *pubsub.PubSub, agentName string, persisted bool,
	objNumCont ObjNumContainer) (*ObjNumPublisher, error) {
...
func (p *ObjNumPublisher) Get(key ObjKey) (ObjNumContainer, error) {
	...
	return covertToContainer(objNum), nil
}
...
func (p *ObjNumPublisher) GetAll() []ObjNumContainer {
	var allObjNums []ObjNumContainer
	for _, objNum := range p.pub.GetAll() {
		allObjNums = append(allObjNums, covertToContainer(objNum))
	}
	return allObjNums
}
...
func covertToContainer(val interface{}) ObjNumContainer {
	...
	return reflectPtr.Interface().(ObjNumContainer)
}
