...
// Example usage:
// queue := cloudconn.CreateDeferredQueue(cloudClient, ...)
//
...
func CreateDeferredQueue(log *base.LogObject, cloudClient *Client,
...
func (q *DeferredQueue) handleDeferred() bool {
	...
	exit := false
	sent := 0
	ctx, cancel := q.cloudClient.GetContextForAllIntfFunctions()
	defer cancel()
	for _, f := range q.priorityCheckFunctions {
		for _, item := range reqs {
			...
			rv, err := q.cloudClient.SendOnAllIntf(ctx, item.url,
				item.buf, RequestOptions{
