...
func Run(ps *pubsub.PubSub, loggerArg *logrus.Logger, logArg *base.LogObject, arguments []string, baseDir string) int { //nolint:gocyclo
	...
	cloudClient := cloudconn.NewClient(log, cloudconn.ClientOptions{
	...
	cloudClient.NoLedManager = true
	log.Functionf("Diag Get Device Serial %s, Soft Serial %s", cloudClient.DevSerial,
		cloudClient.DevSoftSerial)

	// XXX move to later for Get UUID if available

	log.Functionf("diag Run: Use V2 API %v", cloudClient.UsingV2API())

	if fileutils.FileExists(log, types.DeviceCertName) {
		// Load device cert
		cert, err := cloudconn.GetClientCert()
		if err != nil {
			log.Fatal(err)
		}
		ctx.cert = &cert
...
func handleDNSImpl(ctxArg interface{}, key string,
	...
	*ctx.DeviceNetworkStatus = status
	...
	if ctx.cloudClient != nil && ctx.cloudClient.UsingV2API() && ctx.cloudClient.TLSConfig != nil {
		ctx.cloudClient.UpdateTLSProxyCerts()
	}
	if mostlyEqual {
...
func printOutput(ctx *diagContext, caller string) {
	...
	for _, port := range ctx.DeviceNetworkStatus.Ports {
		...
		if !cloudconn.IsExplicitProxyConfigured(port.ProxyConfig) {
...
func printProxy(ctx *diagContext, port types.NetworkPortStatus,
	...
	if cloudconn.IsProxyConfigEmpty(port.ProxyConfig) {
...
func tryPing(ctx *diagContext, ifname string, reqURL string) bool {
	...
	if cloudClient.TLSConfig == nil {
		err := cloudClient.UpdateTLSConfig(ctx.cert)
		if err != nil {
			log.Errorf("internal UpdateTLSConfig failed %v", err)
			return false
		}
		cloudClient.TLSConfig.ClientSessionCache = tls.NewLRUClientSessionCache(0)
	}
	if reqURL == "" {
		reqURL = cloudconn.URLPathString(
			ctx.serverNameAndPort, cloudClient.UsingV2API(), nilUUID, "ping")
	} else {
		// Temporarily change TLS config for the non-controller destination.
		origSkipVerify := cloudClient.TLSConfig.InsecureSkipVerify
		cloudClient.TLSConfig.InsecureSkipVerify = true
		defer func() {
			// Revert back the original TLS config.
			cloudClient.TLSConfig.InsecureSkipVerify = origSkipVerify
		}()
	}
...
func tryPostUUID(ctx *diagContext, ifname string) bool {
	...
	for !done {
		...
		var buf []byte
		reqURL := cloudconn.URLPathString(ctx.serverNameAndPort, cloudClient.UsingV2API(),
			nilUUID, "uuid")
		...
		if senderStatus == types.SenderStatusCertMiss {
			...
			// if controller has cert change. 3) only need to zero out the cache in cloudClient and
			// it will reacquire from the updated cert file. zedagent is the only one responsible for refetching certs.
			cloudClient.ClearCloudCert()
			return false
		}
		retryCount++
		if maxRetries != 0 && retryCount > maxRetries {
...
func parsePrint(resp *http.Response, contents []byte) {
	...
	if err := cloudconn.ValidateProtoContentType(resp); err != nil {
...
func myGet(ctx *diagContext, reqURL string, ifname string,
	...
	proxyURL, err := cloudconn.LookupProxy(log, cloudClient.DeviceNetworkStatus,
		ifname, preqURL)
	if err != nil {
	...
	rv, err := cloudClient.SendOnIntf(context.Background(), reqURL, ifname, nil,
...
func myPost(ctx *diagContext, reqURL string, ifname string,
	...
	rv, err := cloudClient.SendOnIntf(context.Background(), reqURL, ifname, b,
		cloudconn.RequestOptions{
			AllowProxy:     true,
			UseOnboard:     ctx.usingOnboardCert,
			WithNetTracing: withNetTracing,
	...
	if len(rv.RespContents) > 0 {
		err = cloudClient.RemoveAndVerifyAuthContainer(&rv, false)
		if err != nil {
