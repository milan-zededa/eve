...
const (
	...
	// By default, EVE starts renewal attempts after 80% of the certificate’s
...
func Run(ps *pubsub.PubSub, loggerArg *logrus.Logger, logArg *base.LogObject,
	...
	sendTimeoutSecs := scepClient.globalConfig.GlobalValueInt(types.NetworkSendTimeout)
	dialTimeoutSecs := scepClient.globalConfig.GlobalValueInt(types.NetworkDialTimeout)
	scepClient.httpClient = controllerconn.NewClient(scepClient.log,
		controllerconn.ClientOptions{
			AgentName:           agentName,
			DeviceNetworkStatus: scepClient.devNetworkStatus,
			NetworkSendTimeout:  time.Duration(sendTimeoutSecs) * time.Second,
			NetworkDialTimeout:  time.Duration(dialTimeoutSecs) * time.Second,
			ResolverCacheFunc:   scepClient.getCachedResolvedIPs,
			AgentMetrics:        scepClient.agentMetrics,
			DevSerial:           hardware.GetProductSerial(scepClient.log),
			DevSoftSerial:       hardware.GetSoftSerial(scepClient.log),
			DevUUID:             scepClient.devUUID,
			NoLedManager:        true,
			NetTraceOpts: []nettrace.TraceOpt{
				&nettrace.WithLogging{
					CustomLogger: &base.LogrusWrapper{Log: scepClient.log},
				},
	...
	retryPeriodSecs := scepClient.globalConfig.GlobalValueInt(types.SCEPRetryInterval)
	retryAndStartRenew := time.NewTicker(time.Duration(retryPeriodSecs) * time.Second)
