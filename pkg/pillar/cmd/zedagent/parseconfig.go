...
func parseSystemAdapterConfig(config *zconfig.EdgeDevConfig,
	...
	log.Noticef("parseSystemAdapterConfig: Done")
}
...
func validateAndAssignNetPorts(dpc *types.DevicePortConfig, newPorts []*types.NetworkPortConfig) {
	...
	for _, port := range newPorts {
		...
		for _, port2 := range validatedPorts {
			// With VLANs one physicalIO or bond may be used by multiple system adapters.
	...
	for len(propagateFrom) > 0 {
		...
		for _, port := range propagateFrom {
			if port.IsEndpoint || !port.HasError() {
				continue
			}
			l2Refs := invertedRefs[port.Logicallabel]
			if l2Refs == nil {
...
func propagatePhyioAttrsToPort(phyio *types.PhysicalIOAdapter, port *types.NetworkPortConfig) {
...
// Make NetworkPortConfig entry for PhysicalIO which is below a higher-level
// adapter (i.e. it is not an endpoint).
// The port configuration has only those attributes filled which
// are not endpoint-specific.
func makeIntermediatePhyioPort(phyio *types.PhysicalIOAdapter) *types.NetworkPortConfig {
	phyioPort := &types.NetworkPortConfig{Logicallabel: phyio.Logicallabel}
	propagatePhyioAttrsToPort(phyio, phyioPort)
	return phyioPort
}

// Make NetworkPortConfig entry for L2Adapter (VLAN, bond, ...)
// which is below a higher-level adapter (i.e. it is not an endpoint).
// Recursively adds port entries for all adapter below this one.
// The port configuration has only those attributes filled which
// are not endpoint-specific.
func makeIntermediateL2Port(l2Adapter *L2Adapter) (ports []*types.NetworkPortConfig) {
	...
	for _, phyio := range l2Adapter.lowerPhysPorts {
		ports = append(ports, makeIntermediatePhyioPort(phyio))
	}
	for _, lowerL2 := range l2Adapter.lowerL2Ports {
		ports = append(ports, makeIntermediateL2Port(lowerL2)...)
	}
	return ports
}
...
// One of the returned ports is endpoint (IsEndpoint=true), others are intermediaries
// with only some of the NetworkPortConfig attributes set.
...
func parseOneSystemAdapterConfig(getconfigCtx *getconfigContext,
	...
	port.IsEndpoint = true // this one has SystemAdapter directly attached to it
	...
	if phyio != nil {
		...
		propagatePhyioAttrsToPort(phyio, port)
	} else {
		...
		// Add NetworkPortConfig entries for intermediate adapters.
		for _, phyio := range l2Adapter.lowerPhysPorts {
			ports = append(ports, makeIntermediatePhyioPort(phyio))
		}
		for _, lowerL2 := range l2Adapter.lowerL2Ports {
...
func parseDeviceIoListConfig(config *zconfig.EdgeDevConfig,
	...
	for indx, ioDevicePtr := range deviceIoList {
		...
		if port.Logicallabel == "" {
			...
			// Originally, the physical label was used as the IO adapter identifier
...
func parseBonds(config *zconfig.EdgeDevConfig, getconfigCtx *getconfigContext) bool {
	...
	for _, bondConfig := range config.GetBonds() {
		...
		if len(portConfig.Bond.AggregatedPorts) == 0 {
			// We will re-check when phyio change.
			errStr := fmt.Sprintf("Missing all lower-layer adapters for bond %s; ignored",
...
func parseVlans(config *zconfig.EdgeDevConfig, getconfigCtx *getconfigContext) bool {
	...
	for _, vlanConfig := range config.GetVlans() {
		...
		if portConfig.VLAN.ParentPort == "" {
			// We will re-check when phyio or bonds change.
			errStr := fmt.Sprintf("Missing lower-layer adapter for VLAN %s; ignored",
				portConfig.Logicallabel)
			log.Errorf("parseBonds: %s", errStr)
			continue
		}
