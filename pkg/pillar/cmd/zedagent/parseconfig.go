...
func parseSCEPProfiles(getconfigCtx *getconfigContext, config *zconfig.EdgeDevConfig) bool {
	...
	for _, profile := range config.GetScepProfiles() {
		...
		// Check for duplicate SCEP profile name.
		_, err := getconfigCtx.pubSCEPProfile.Get(profile.GetProfileName())
		if err == nil {
			errStr := fmt.Sprintf("Duplicate SCEP profile name %q; "+
				"ignoring config: %+v", profile.GetProfileName(), profile)
