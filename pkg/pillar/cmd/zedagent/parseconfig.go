...
func parseNetworkWirelessConfig(ctx *getconfigContext, key string, netEnt *zconfig.NetworkConfig) types.WirelessConfig {
	...
	case zconfig.WirelessType_Cellular:
		...
		if len(cellNetConfig.AccessPoints) == 0 && cellNetConfig.APN != "" {
			...
			// Just for demo.
			if strings.Contains(cellNetConfig.APN, "://") {
				// Format: <proto>://<username>:<password>@apn
				splitProto := strings.Split(cellNetConfig.APN, "://")
				if len(splitProto) == 2 {
					proto := strings.ToLower(splitProto[0])
					switch proto {
					case "pap":
						ap.AuthProtocol = types.WwanAuthProtocolPAP
					case "chap":
					...
					splitUsername := strings.Split(splitProto[1], ":")
					if len(splitUsername) == 2 {
						ap.CleartextUsername = splitUsername[0]
						splitPassword := strings.Split(splitUsername[1], "@")
						if len(splitPassword) == 2 {
							ap.CleartextPassword = splitPassword[0]
							ap.APN = splitPassword[1]
						}
					}
				}
			}
