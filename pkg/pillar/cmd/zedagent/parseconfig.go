...
func parseIpspec(ipspec *zconfig.Ipspec,
	...
	// Currently, we cannot use more than types.BitmapMax IPs for dynamic allocation
	...
	// what ByteAllocator based on Bitmap can provide) and use it for network instances
	// that require bigger subnets.
	if addressesInRange > objtonum.BitmapMax {
		config.DhcpRange.End = types.AddToIP(config.DhcpRange.Start, objtonum.BitmapMax)
	}
	return nil
}
