...
func initNumberAllocators(ctx *zedrouterContext, ps *pubsub.PubSub) {
	...
	appAndBridgePublisher, err := objtonum.NewObjNumPublisher(
		ps, agentName, true, &types.UuidToNum{})
	if err != nil {
		log.Fatal(err)
	}
	...
	for _, item := range pubAppNetworkStatus.GetAll() {
		...
		allocatedNum, err := ctx.appNumAllocator.GetOrAllocate(
			appNumKey, objtonum.RequireNumber{Number: appNum})
		if err != nil {
			log.Errorf(
				"UuidToNum and AppNetworkStatus are out of sync for key %s: %d vs. %d",
				appNumKey.Key(), allocatedNum, appNum)
		}
	}
	...
	for _, item := range ctx.pubNetworkInstanceStatus.GetAll() {
		...
		// Remove reserved-only flag.
		allocatedNum, err := ctx.bridgeNumAllocator.GetOrAllocate(
			bridgeNumKey, objtonum.RequireNumber{Number: bridgeNum})
		if err != nil {
			log.Errorf(
				"UuidToNum and NetworkInstanceStatus are out of sync for key %s: %d vs. %d",
				bridgeNumKey.Key(), allocatedNum, bridgeNum)
		}
	}
	...
	ctx.appIntfNumPublisher, err = objtonum.NewObjNumPublisher(
		ps, agentName, true, &types.AppInterfaceToNum{})
	if err != nil {
		log.Fatal(err)
	}
	...
	if allocCount > 0 {
		return fmt.Errorf(
			"cannot delete app-interface allocator for network instance %s: "+
				"the set of allocated numbers is not empty", netInstID)
	}
