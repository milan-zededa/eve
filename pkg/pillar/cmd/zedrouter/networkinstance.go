...
func (z *zedrouter) setSelectedUplink(uplinkLogicalLabel string,
	...
	switch len(ports) {
	case 0:
		err = fmt.Errorf("label of selected uplink does not match any port (%v)", ports)
		// Wait for DPC update
		return true, err
	case 1:
		// OK
...
func (z *zedrouter) doUpdateNIUplink(uplinkLogicalLabel string,
	...
	_, err := z.setSelectedUplink(uplinkLogicalLabel, status)
	if err != nil {
	...
	if status.ActivationPending() && status.WaitingForUplink {
...
func (z *zedrouter) doActivateNetworkInstance(config types.NetworkInstanceConfig,
	...
	if err == nil {
		err = z.niStateCollector.StartCollectingForNI(config, br, vifs)
	}
	if err != nil {
		z.log.Error(err)
	}
