...
var (
	...
	reachProber *portprober.MockReachProber
)
...
func initportprober(t *GomegaWithT, intfs selectedIntfs) {
	...
	*/
	...
	reachProber = portprober.NewMockReachProber()
	setDefaultReachState(intfs)
	prober = portprober.NewPortProber(logObj, configForTest(), reachProber)
...
func setDefaultReachState(intfs selectedIntfs) {
	if intfs.eth0 {
		reachProber.SetReachabilityState(portIfName(eth0LL), eth0NHAddr, nil, 50*time.Millisecond)
	}
	if intfs.eth1 {
		reachProber.SetReachabilityState(portIfName(eth1LL), eth1NHAddr, nil, 50*time.Millisecond)
	}
	if intfs.wlan0 {
		reachProber.SetReachabilityState(portIfName(wlanLL), wlan0NHAddr, nil, 100*time.Millisecond)
	}
	if intfs.wwan0 {
		reachProber.SetReachabilityState(portIfName(wwan0LL), wwan0NHAddr, nil, 200*time.Millisecond)
	}
	if intfs.wwan1 {
		reachProber.SetReachabilityState(portIfName(wwan1LL), wwan1NHAddr, nil, 300*time.Millisecond)
	}
}
...
func eventuallyProbe(t *GomegaWithT, portLL string, dstAddr net.Addr) {
	startTime := time.Now()
	probeDone := func() bool {
		probeTime := reachProber.LastProbe(portIfName(portLL), dstAddr)
...
func TestProbingSingleRoute(test *testing.T) {
	...
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbeAddr, nil, 250*time.Millisecond)
	...
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbeAddr, nil, 250*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan0LL), userProbeAddr, nil, 400*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan1LL), userProbeAddr, nil, 500*time.Millisecond)
	...
	reachProber.SetReachabilityState(portIfName(eth0LL), eth0NHAddr, errors.New("unreachable"), 50*time.Millisecond)
	...
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbeAddr, nil, time.Minute)
	...
	reachProber.SetReachabilityState(portIfName(wwan0LL), userProbeAddr, errors.New("unreachable"), 50*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan1LL), userProbeAddr, errors.New("unreachable"), 50*time.Millisecond)
...
func TestProbingMultipleRoutes(test *testing.T) {
	...
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbe1Addr, nil, 250*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan0LL), userProbe1Addr, nil, 400*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan1LL), userProbe1Addr, nil, 400*time.Millisecond)
	...
	reachProber.SetReachabilityState(portIfName(eth1LL), userProbe2Addr, nil, 250*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wlanLL), userProbe2Addr, nil, 350*time.Millisecond)
	...
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbe2Addr, nil, 250*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan0LL), userProbe2Addr, nil, 400*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan1LL), userProbe2Addr, nil, 400*time.Millisecond)
	...
	reachProber.SetReachabilityState(portIfName(eth1LL), eth1NHAddr, errors.New("unreachable"), 50*time.Millisecond)
...
func TestProbingNoConnectivity(test *testing.T) {
	...
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbeAddr, unresponsive, 50*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(eth0LL), eth0NHAddr, unresponsive, 50*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wlanLL), userProbeAddr, unresponsive, 100*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wlanLL), wlan0NHAddr, unresponsive, 100*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan0LL), userProbeAddr, unresponsive, 200*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wwan0LL), wwan0NHAddr, unresponsive, 200*time.Millisecond)
...
func TestPortCostChange(test *testing.T) {
	...
	userProbeAddr := &portprober.HostnameAddr{Hostname: "webserver", Port: 80}
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbeAddr, nil, 50*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wlanLL), userProbeAddr, nil, 100*time.Millisecond)
...
func TestAvoidStateFlapping(test *testing.T) {
	...
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbeAddr, nil, time.Millisecond)
	reachProber.SetReachabilityState(portIfName(eth0LL), eth0NHAddr, nil, time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wlanLL), userProbeAddr, nil, time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wlanLL), wlan0NHAddr, nil, time.Millisecond)
	...
	for i := 0; i < 10; i++ {
		if i%2 == 0 {
			...
			unresponsive := errors.New("unresponsive")
			reachProber.SetReachabilityState(portIfName(eth0LL), eth0NHAddr, unresponsive, 50*time.Millisecond)
			reachProber.SetReachabilityState(portIfName(eth0LL), userProbeAddr, unresponsive, 250*time.Millisecond)
		} else {
...
func TestDisappearedPort(test *testing.T) {
	...
	userProbeAddr := &portprober.HostnameAddr{Hostname: "webserver", Port: 80}
	reachProber.SetReachabilityState(portIfName(eth0LL), userProbeAddr, nil, 250*time.Millisecond)
	reachProber.SetReachabilityState(portIfName(wlanLL), userProbeAddr, nil, 250*time.Millisecond)
