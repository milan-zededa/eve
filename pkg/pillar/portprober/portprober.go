...
// There might be a different implementation for every supported network stack (currently
// only Linux networking is supported by EVE).
...
type ReachabilityProber interface {
	...
	Probe(ctx context.Context, portIfName string, srcIP net.IP, dstAddr net.Addr,
		method types.ConnectivityProbeMethod, dnsServers []net.IP) error
}
...
func (ps portStatus) matchesLabels(niLabel, routeLabel string) bool {
	return (ps.logicallabel == niLabel || generics.ContainsItem(ps.sharedlabels, niLabel)) &&
		(ps.logicallabel == routeLabel || generics.ContainsItem(ps.sharedlabels, routeLabel))
}
...
func (p *PortProber) StartPortProbing(ni uuid.UUID, niPortLabel string,
	...
	for _, port := range p.ports {
		if !port.matchesLabels(niPortLabel, mpRoute.OutputPortLabel) {
			continue
		}
...
func (p *PortProber) StopPortProbing(ni uuid.UUID, mpRouteDst *net.IPNet) error {
	...
	for _, port := range p.ports {
		if !port.matchesLabels(mpRoute.NIPortLabel, mpRoute.MPRoute.OutputPortLabel) {
			continue
		}
		if probeCfg.EnabledGwPing && port.cost <= probeCfg.GwPingMaxCost {
			port.nhProbe.refCount--
...
func (p *PortProber) probePortNH(port *portStatus) {
	...
	for _, nhIP := range port.nextHops {
		...
		for _, localIP := range port.localAddrs {
			...
			err = p.reachProber.Probe(ctx, port.ifName, localIP, nhAddr,
				types.ConnectivityProbeMethodICMP, nil)
			cancel()
			if err == nil {
				break
			}
...
func (p *PortProber) probePortUserEp(port *portStatus, probe types.ConnectivityProbe) {
	...
	for _, localIP := range port.localAddrs {
		...
		startTime := time.Now()
		err = p.reachProber.Probe(ctx, port.ifName, localIP, dstAddr, probe.Method,
			port.dnsServers)
...
func (p *PortProber) pickPortForRoute(routeKey string) (changed bool) {
	...
	if anyUPState {
		...
		for portLL, port := range p.ports {
			...
			if route.SelectedPortLL == portLL {
				return false
			}
			ports = append(ports, portLL)
		}
		// 2.4. Try to share load between these ports using round-robin approach.
		selectedPort := p.roundRobinPick(ports)
		if route.SelectedPortLL == "" {
			p.log.Noticef(
	...
	for portLL, port := range p.ports {
		...
		for _, ip := range port.localAddrs {
			if ip.IsGlobalUnicast() {
				matchingPorts = append(matchingPorts, portLL)
			}
		}
	}
	if len(matchingPorts) > 0 {
		if route.MPRoute.PreferLowerCost {
			matchingPorts = p.lowestCostPorts(matchingPorts)
		}
		route.SelectedPortLL = p.roundRobinPick(matchingPorts)
		p.log.Noticef(
			"PortProber: Selecting port %s for route dst=%v NI=%v "+
