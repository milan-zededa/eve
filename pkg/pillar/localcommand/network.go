...
func (lc *LocalCmdAgent) initializeNetworkConfig() {
	lc.Log.Noticef("HEY! initializeNetworkConfig BEGIN")
	lc.networkTicker = newTaskTicker(networkPOSTInterval)
	lc.networkConfig = types.DevicePortConfig{
		Version: types.DPCIsMgmt,
		Key:     "lps",
	}
	config, err := lc.readSavedNetworkConfig()
	lc.Log.Noticef("HEY! readSavedNetworkConfig: %+v, %v", config, err)
	if err != nil {
	...
	lc.Log.Noticef("HEY! initializeNetworkConfig END")
}

// runNetworkTask continuously publishes the current network configuration and potentially
// retrieves a locally-made network configuration for a subset of ports from LPS
...
//
// Returns:
//   - A LocalNetworkConfig if LPS responds with 200/201 and the token matches.
//   - Nil if LPS is not configured, token is invalid, no addresses are available,
...
//   - 200/201: return local config if the LPS token is valid.
	...
	for _, port := range dpc.Ports {
		...
		case types.WirelessTypeCellular:
			...
			cellConfig := &profile.CellularConfig{
				AttachBearerAuth: &profile.CellularNetworkAuth{
					AuthProtocol: 0,
				},
...
func (lc *LocalCmdAgent) applyLocalNetworkConfig(
	...
	newNetworkConfig := types.DevicePortConfig{
		Version: types.DPCIsMgmt,
		Key:     "lps",
	}
...
func (lc *LocalCmdAgent) networkPortConfigFromProto(protoConfig *profile.NetworkPortConfig,
	...
	if len(protoConfig.GetGateways()) > 0 {
	...
	if len(protoConfig.GetGateways()) == 1 {
		...
		if ipAddr == nil {
			err = fmt.Errorf("port %q: invalid gateway IP address %q: %w",
				logicalLabel, gatewayIP, err)
			lc.Log.Errorf("%s: networkPortConfigFromProto: %v (proto=%v)",
				logPrefix, err, protoConfig)
			return nil, err
		}
	...
	for _, dnsServer := range protoConfig.GetDnsServers() {
		...
		if ipAddr == nil {
			err = fmt.Errorf("port %q: invalid DNS server IP address %q: %w",
				logicalLabel, dnsServer, err)
			lc.Log.Errorf("%s: networkPortConfigFromProto: %v (proto=%v)",
				logPrefix, err, protoConfig)
			return nil, err
		}
...
func (lc *LocalCmdAgent) readSavedNetworkConfig() (types.DevicePortConfig, error) {
	...
	lc.Log.Noticef("HEY! ReadSavedConfig: %+v, %v, %v", contents, ts, err)
	if err != nil {
		return config, err
	}
	if contents != nil {
