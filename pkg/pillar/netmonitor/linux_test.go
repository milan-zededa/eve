...
func TestParseDHCPv4Lease(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantNTP: []net.IP{net.ParseIP("132.163.96.5")},
		},
		{
		...
		{
			name: "Invalid NTP address",
			leaseData: `
				network_number=192.168.1.0
				subnet_cidr=24
				ntp_servers=invalid.ip 1.2.3.4
			`,
			wantSubnet: &net.IPNet{
				IP:   net.IPv4(192, 168, 1, 0),
				Mask: net.CIDRMask(24, 32),
			},
			wantNTP: []net.IP{net.ParseIP("1.2.3.4")},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			subnet, ntp, err := netmonitor.ParseDHCPv4Lease(tt.leaseData)
			if err != nil {
