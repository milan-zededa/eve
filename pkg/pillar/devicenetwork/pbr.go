...
const (
	...
	// connectivity (between EVE and remote endpoints, such as the controller; configured
	// via DPC - Device Port Configuration).
...
func FlushRules(log *base.LogObject, ifindex int) {
	...
	for _, r := range rules {
		if r.Table != DPCBaseRTIndex+ifindex {
			continue
		}
...
func makeSrcNetlinkRule(ifindex int, p net.IPNet, addForSubnet bool, prio int) *netlink.Rule {
	r := netlink.NewRule()
	r.Table = DPCBaseRTIndex + ifindex
	r.Priority = prio
