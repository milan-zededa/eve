...
type DeviceNetworkContext struct {
	...
	AirplaneMode           types.AirplaneMode
...
func RestartVerify(ctx *DeviceNetworkContext, caller string) {
	...
	if !ctx.AirplaneMode.PermanentlyEnabled && !ctx.AirplaneMode.InProgress &&
		ctx.AirplaneMode.Enabled {
		log.Noticef("RestartVerify: Airplane-mode is enabled, skipping DPC verification")
		return
	}
...
func handleZedAgentStatusImpl(ctxArg interface{}, key string, statusArg interface{}) {
	...
	if newStatus.AirplaneMode.RequestedAt.After(ctx.AirplaneMode.RequestedAt) {
		log.Noticef("Switching airplane mode %s", newStatus.AirplaneMode.StateAsString())
		updateAirplaneMode(ctx, newStatus.AirplaneMode)
	}
}
