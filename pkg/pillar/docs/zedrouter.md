...
To determine the state of connectivity for a given port, PortProber uses `ReachabilityProber`.
This is separated from PortProber via Go interface to allow different implementations
for different network stacks. The default implementation is `LinuxReachabilityProber`,
which performs probing attempts from inside the Linux network stack.
Additionally, there is `MockReachProber`, which implements `ReachabilityProber` for the sake
of PortProber unit testing.
Note that every implementation of `ReachabilityProber` should support all probing methods
defined by EVE API. Currently, this includes ICMP and TCP probing.
