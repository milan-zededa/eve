...
// PhysIf : network interface corresponding to a physical NIC (Ethernet or WiFi).
...
func (c *PhysIfConfigurator) Create(_ context.Context, item depgraph.Item) error {
	...
	return c.updateMTU(physIf.PhysIfName, physIf.GetMTU())
}

func (c *PhysIfConfigurator) updateMTU(ifName string, mtu uint16) error {
...
func (c *PhysIfConfigurator) Delete(_ context.Context, item depgraph.Item) error {
	...
	return c.updateMTU(physIf.PhysIfName, types.DefaultMTU)
}
