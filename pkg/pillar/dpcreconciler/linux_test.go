...
func TestSingleEthInterface(test *testing.T) {
	...
	t.Expect(status.RS.ConfigError).To(BeEmpty())
	t.Expect(status.DNS.Error).To(BeNil())
	t.Expect(status.DNS.Servers).To(HaveKey("mock-eth0"))
	t.Expect(status.DNS.Servers["mock-eth0"]).To(BeEmpty())
	...
	t.Expect(status.DNS.Error).To(BeNil())
	t.Expect(status.DNS.Servers).To(HaveKey("mock-eth0"))
	t.Expect(status.DNS.Servers["mock-eth0"]).To(HaveLen(1))
	t.Expect(status.DNS.Servers["mock-eth0"][0].String()).To(Equal("8.8.8.8"))
