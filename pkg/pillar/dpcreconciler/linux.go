...
func (r *LinuxDpcReconciler) getIntendedWwanConfig(dpc types.DevicePortConfig,
	...
	for _, port := range dpc.Ports {
		...
		if accessPoint == nil {
			r.Log.Warnf("getIntendedWwanConfig: no activated access point for port %s, "+
		...
		// Just for demo.
		if decBlock.CellNetUsername == "" && accessPoint.CleartextUsername != "" {
			decBlock.CellNetUsername = accessPoint.CleartextUsername
			decBlock.CellNetPassword = accessPoint.CleartextPassword
		}
