...
func (r *LinuxDpcReconciler) updateCurrentRoutes(dpc types.DevicePortConfig) (changed bool) {
	...
	prevSG := dg.GetSubGraph(r.currentState, sgPath)
	diff := prevSG.DiffItems(currentRoutes)
	if len(diff) > 0 {
		r.Log.Noticef("HEY!!! We have current-state route differences: %v", diff)
		for _, itemRef := range diff {
			prevItem, _, _, found := prevSG.Item(itemRef)
			r.Log.Noticef("Prev item: %v, %t", prevItem, found)
			newItem, _, _, found := currentRoutes.Item(itemRef)
			r.Log.Noticef("New item: %v, %t", newItem, found)
			if prevItem != nil && newItem != nil {
				r.Log.Noticef("Equal?: %t", prevItem.Equal(newItem))
				r.Log.Noticef("Prev netlink route: %#v", prevItem.(linux.Route).Route)
				r.Log.Noticef("New netlink route: %#v", newItem.(linux.Route).Route)
			}
		}
	}
