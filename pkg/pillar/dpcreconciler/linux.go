...
func (r *LinuxDpcReconciler) rebuildRouteMetricMap(dpc types.DevicePortConfig) {
	...
	sort.SliceStable(pi, func(i, j int) bool {
		if pi[i].isMgmt != pi[j].isMgmt {
			return pi[i].isMgmt // mgmt pi first
		}
		if pi[i].cost != pi[j].cost {
			return pi[i].cost < pi[j].cost
		}
		return pi[i].index < pi[j].index
	})
