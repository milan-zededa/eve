...
// Logger : interface of a logger that depgraph will use (if provided)
// to log some important events happening inside the graph.
...
func NewDepGraph(log Logger) DepGraph {
...
func (g *depGraph) RegisterConfigurator(configurator Configurator, itemType string) error {
	...
	if duplicate {
		return fmt.Errorf("multiple configurators registered for item type: %s",
			itemType)
	}
...
func (g *depGraph) syncItems(ctx context.Context) (metrics syncMetrics, errs []error) {
	...
	for _, itemChange := range g.plannedChanges.items {
		...
		if exists {
			...
			g.addNewNode(itemName, newClusterPath, newValue)
		}
	...
	for !stage1Stack.isEmpty() {
		...
		if node.needSync && !node.value.Equal(node.newValue) {
			var deps []Dependency
			if !external {
				deps = node.newValue.Dependencies()
				validateDeps(deps)
			}
...
func (g *depGraph) syncClusterInfo() {
	for _, cluster := range g.plannedChanges.clusters {
		...
		for {
			...
			_, exists = g.clusters[parentPath]
			if exists {
				break
			}
			info = &clusterInfo{
...
func (g *depGraph) addNewNode(nodeName, clusterPath string, value Item) *node {
	...
	if value.External() && len(deps) > 0 {
		panic(fmt.Sprintf("external item %s should not have dependencies",
			value.Name()))
	}
	validateDeps(deps)
