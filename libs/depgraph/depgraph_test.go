...
func newTestCtx(t *testing.T) *testCtx {
	return &testCtx{
		graph:  depgraph.NewDepGraph(logger{t: t}),
		opsLog: &operationsLog{},
	}
}

func newBenchCtx(b *testing.B) *testCtx {
	return &testCtx{
		graph:  depgraph.NewDepGraph(logger{b: b}),
		opsLog: &operationsLog{},
	}
}
...
func (ctx *testCtx) addConfigurator(forItemType string) error {
...
func TestItemsWithoutDependencies(t *testing.T) {
	...
	g.Expect(ctx.addConfigurator("type1")).To(Succeed())
	g.Expect(ctx.addConfigurator("type2")).To(Succeed())
	...
	g.Expect(summary.Item).To(BeMockItem(itemA))
	...
	g.Expect(exists).To(BeTrue())
	g.Expect(summary.Item).To(BeMockItem(itemB))
...
func TestDependencyItemIsCreated(t *testing.T) {
	...
	summary, exists = ctx.graph.Item(itemC.name).Get()
	g.Expect(exists).To(BeTrue())
	g.Expect(summary.Item).To(BeMockItem(itemC))
	g.Expect(summary.LastOp).To(Equal(depgraph.OperationCreate))
	g.Expect(summary.LastError).To(BeNil())
	g.Expect(summary.State).To(Equal(depgraph.ItemStateCreated))
...
func TestRecreateWhenModified(t *testing.T) {
	...
	itemA := mockItem{
		...
		depsCb: depFromStrAttrWithRecreate,
	}
	itemB := mockItem{
		name:     "B",
		itemType: "type1",
...
func TestTransitiveDependencies(t *testing.T) {
	...
	g.Expect(ctx.addConfigurator("type1")).To(Succeed())
	g.Expect(ctx.addConfigurator("type2")).To(Succeed())
	g.Expect(ctx.addConfigurator("type3")).To(Succeed())
...
// use-case.
func perfTest(b *testing.B, numOfItems int) error {
	ctx = newBenchCtx(b)
	err := ctx.addConfigurator("item-type")
	if err != nil {
		return err
	}
	const numOfDeps = 10
