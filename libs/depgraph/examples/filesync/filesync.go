...
const graphvizOnline = "https://dreampuf.github.io/GraphvizOnline"
...
`, dir)
	...
	dot, err := g.RenderDOT()
	if err != nil {
		log.Errorf("depgraph DOT rendering failed: %v", err)
		os.Exit(1)
	}
	printHeader("Applied the intended state:")
	fmt.Println(description)
	printHeader("depgraph DOT rendering (visualize online using %s):", graphvizOnline)
	fmt.Println(dot)
	printHeader("Verify the content of %s and press ENTER to continue", dir)
	_, _ = fmt.Scanln()
...
`, dir)
	...
	printHeader("Removed root from the graph.")
	printHeader("All files and directories under %s should be removed "+
		"and in the pending state.", dir)
	printHeader("depgraph DOT rendering (visualize online using %s):", graphvizOnline)
	fmt.Println(dot)
	printHeader("Verify the content of %s and press ENTER to exit", dir)
	_, _ = fmt.Scanln()
}
