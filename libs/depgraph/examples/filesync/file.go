...
// Equal returns false even if only the parent directory reference changes.
...
func (fc fileConfigurator) Create(ctx context.Context, item depgraph.Item) error {
	f := item.(file)
...
func (fc fileConfigurator) Modify(ctx context.Context, oldItem, newItem depgraph.Item) (err error) {
	oldF := oldItem.(file)
	newF := newItem.(file)
...
func (fc fileConfigurator) Delete(ctx context.Context, item depgraph.Item) error {
	f := item.(file)
	return os.Remove(f.path())
}

// NeedsRecreate returns true when the file content changes.
// This is used just to demonstrate how depgraph can re-create an item.
...
// DependsOn returns the parent directory as the file's only dependency.
func (fc fileConfigurator) DependsOn(item depgraph.Item) []depgraph.Dependency {
	f := item.(file)
