...
// (which is created with MkdirTemp outside of the graph).
...
func (dc dirConfigurator) Modify(ctx context.Context, oldItem, newItem depgraph.Item) (err error) {
...
func (dc dirConfigurator) Delete(ctx context.Context, item depgraph.Item) error {
	d := item.(directory)
	return os.Remove(d.Name())
}

func (dc dirConfigurator) NeedsRecreate(oldItem, newItem depgraph.Item) (recreate bool) {
	return false
}

// DependsOn returns the parent directory as the only dependency.
func (dc dirConfigurator) DependsOn(item depgraph.Item) []depgraph.Dependency {
	d := item.(directory)
	if d.parent == nil {
