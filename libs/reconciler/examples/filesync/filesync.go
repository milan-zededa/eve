...
`, rootDirname)
	...
	// 3. Finally, remove the root from the graph of the current state, essentially
	//    to announce that it will be removed (outside of Reconciler). Since everything
	//    either directly or transitively depends on it, all files and directories
	//    will be removed and marked as pending (ItemStatePending).
	d.currentState.DelNode(depgraph.NodeIDFor(rootDir))
	r = reconciler.New(d.registry)
	status = r.Reconcile(context.Background(), d.currentState, d.intendedState)
	if status.Err != nil {
		log.Fatalf("State reconciliation failed: %v", status.Err)
	...
	// Inform the user.
	d.printReport("Removed root from the graph.")
	d.printReport("All files and directories under %s should be removed "+
		"and in the pending state.", rootDirname)
	d.printReport("Visualization of the graph with the current state: %s ",
		gvRedirectURL+gvCurrentState)
	d.printReport("Visualization of the graph with the intended state: %s ",
		gvRedirectURL+gvIntendedState)
