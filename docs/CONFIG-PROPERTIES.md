...
| kubernetes.vmi.deschedule.events | string | "" | - | - | Comma-separated list of events that trigger VMI descheduling. `boot` runs the descheduler once per boot, which returns an app to its designated node after that node has been away. `join` moves an app onto this node when it newly becomes the app's designated node while already running (e.g. after a controller-driven `designated_node_id` reassignment) - a live cluster join never restarts the agent, so it needs its own trigger distinct from boot, and neither the scheduler nor the generic descheduler policy ever revisits an already-running pod on their own. When empty (default), no event-driven descheduling is performed. |
