...
- **Switch**:
  ...
  - currently can be connected to one port
...
Uplink denotes network adapter used for EVE management, i.e. providing connectivity
with the controller. Multiple adapters can be used for management and EVE will automatically
fail-over between them when some loose connectivity.
...
- If flow logging is enabled, ACLs are implemented by iptables `mangle` table for the most
  ...
  and routed into a dummy `blackhole` interface
