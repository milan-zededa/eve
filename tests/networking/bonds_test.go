...
func TestActiveBackupBond(test *testing.T) {
	...
	// slave transmits at a time, so no switch-side LAG/bond is needed.
	...
	t.Eventually(devUpdates, timeout).Should(Receive(matchers.SatisfyPredicate(
		...
		func(info *eveinfo.ZInfoDevice) bool {
			...
			if bondStatus == nil || len(bondStatus.GetSlaves()) != 2 {
				return false
			}
			activeSlave = bondStatus.GetActiveSlave()
			return activeSlave != ""
		})))
	...
	evetest.Logger().Infof("Currently active slave: %s", activeSlave)
	t.Expect(activeSlave).To(BeElementOf("ethernet0", "ethernet1"))
	...
	// Simulate link failure on the active slave by setting AdminUp=false
	// in the network model.
	// Map the active slave logical label to the SDN port index.
	var activeSlaveIdx int
	switch activeSlave {
	case "ethernet0":
		activeSlaveIdx = 0
	case "ethernet1":
	...
	updatedModel.Ports[activeSlaveIdx].AdminUp = false
	evetest.UpdateNetworkModel(updatedModel)
	evetest.Checkpoint("active-slave-link-down")
	...
	// Verify the active slave has changed and the failed slave reports MII down.
	var newActiveSlave string
	t.Eventually(devUpdates, timeout).Should(Receive(matchers.SatisfyPredicate(
		"Bond failover to different slave with MII down on failed slave",
		func(info *eveinfo.ZInfoDevice) bool {
			port := getDevicePort("active-backup-bond", info)
			if port == nil {
				return false
			...
			newActiveSlave = bs.GetActiveSlave()
			if newActiveSlave == "" || newActiveSlave == activeSlave {
				return false
			}
			// Check that the original active slave (now failed) has MII down.
			for _, slave := range bs.GetSlaves() {
				if slave.GetLogicallabel() == activeSlave {
					return !slave.GetMiiUp()
				}
			}
			return false
		})))
	evetest.Logger().Infof("Active slave after failover: %s", newActiveSlave)
	evetest.Checkpoint("failover-verified")

	// Verify that bond metrics report a non-zero link failure count
	// for the slave that went down.
	t.Eventually(devMetrics, timeout).Should(Receive(matchers.SatisfyPredicate(
		"Bond metrics report link failure for failed slave",
		func(metrics *evemetrics.DeviceMetric) bool {
			for _, bm := range metrics.GetBondMetrics() {
				if bm.GetLogicallabel() != "active-backup-bond" {
					continue
				}
				for _, slave := range bm.GetSlaves() {
					if slave.GetLogicallabel() == activeSlave &&
						slave.GetLinkFailureCount() > 0 {
						return true
					}
				}
			}
...
func TestLACPBond(test *testing.T) {
	...
	// Verify all slaves are in the active aggregator (no split aggregation).
	for _, slave := range bondStatus.GetSlaves() {
		t.Expect(slave.GetLogicallabel()).To(BeElementOf("ethernet0", "ethernet1"))
		t.Expect(slave.GetMiiUp()).To(BeTrue())
		t.Expect(slave.GetLacp().GetAggregatorId()).To(Equal(activeAggID))
	}
	...
	// Verify that bond metrics are reported with both slaves and LACP sub-metrics.
	t.Eventually(devMetrics, timeout).Should(Receive(matchers.SatisfyPredicate(
		"Bond metrics report both slaves with LACP metrics",
		func(metrics *evemetrics.DeviceMetric) bool {
			for _, bm := range metrics.GetBondMetrics() {
				if bm.GetLogicallabel() != "lacp-bond" {
					continue
				}
				var slaveLabels []string
				for _, slave := range bm.GetSlaves() {
					if slave.GetLacp() == nil {
						return false
					}
					slaveLabels = append(slaveLabels, slave.GetLogicallabel())
				}
				return generics.EqualSets(slaveLabels,
