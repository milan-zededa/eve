...
const (
	...
	// Maximum allowed side for one EVE log entry.
	logBufferMaxCapacity = 1024 * 1024
)
...
// InfoMsgWriter consumes device informational messages (ZInfoMsg).
// Implementations define how messages are handled (e.g., written to a file,
// stored, or forwarded). Write should return an error if the message
// cannot be processed.
type InfoMsgWriter interface {
	Write(msg *eveinfo.ZInfoMsg) error
}
...
func (ac *AdamClient) Start() error {
	...
	if err != nil {
		err = fmt.Errorf("failed to open Adam output file %q: %w", outputFile, err)
		ac.publish(AdamStateCrashed, err)
		return err
	}
...
func (ac *AdamClient) WaitUntilDevRequest(
	...
	// Child context to stop stream when we exit.
	...
	go func() {
		defer wg.Done()
		ac.streamAPIRequests(streamCtx, devUUID, ch)
	}()
...
// streamAPIRequests establishes a long-lived streaming connection to Adam
// to observe API requests originating from the specified device.
//
// It continuously reconnects on transient failures with a fixed retry delay.
// Decoded request events are forwarded to eventCh until ctx is canceled.
// The function returns when ctx is done or when a non-recoverable error occurs.
func (ac *AdamClient) streamAPIRequests(ctx context.Context, devUUID uuid.UUID,
	...
	for {
		...
		req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
		if err != nil {
			ac.log.Errorf("failed to create GET %s request: %v", url, err)
			continue
		}
		...
		if err != nil {
			ac.log.Errorf("failed to issue GET %s: %v", url, err)
			continue
		}
		...
		func() {
			...
			if resp.StatusCode != http.StatusOK {
				ac.log.Errorf("unexpected HTTP status %s from %s", resp.Status, url)
				return
			}
...
func (ac *AdamClient) SubscribeToDeviceLogs(
	...
	go func() {
		defer wg.Done()
		ac.streamDeviceLogs(streamCtx, devUUID, match, channel)
	}()
...
// streamDeviceLogs maintains a long-lived streaming connection to Adam in
// order to observe log entries originating from the specified device.
//
// The function automatically reconnects after transient failures using a
// fixed retry delay. Each received log record is decoded and, if it matches
// the provided filter, forwarded to logCh.
//
// The function runs until ctx is canceled. When it exits, logCh is closed to
// signal that no further log entries will be delivered.
//
// This function is intended to be executed in its own goroutine.
func (ac *AdamClient) streamDeviceLogs(
	ctx context.Context, devUUID uuid.UUID, match logger.LogEntryMatcher,
	logCh chan<- *evelogs.LogEntry) {
	const retryDelay = 3 * time.Second
	defer close(logCh)
		...
		func() {
			...
			for {
				...
				if err = protojson.Unmarshal(raw, entry); err != nil {
					ac.log.Errorf(
						"failed to proto-unmarshal streamed log entry: %v",
						err,
					)
					continue
				}
...
// WriteDeviceLogs retrieves all matching logs published so far by the given device
// and writes them using the provided DeviceLogWriter.
//
// If match is non-nil, only logs for which match(entry) returns true are written.
// If match is nil, all messages are written.
//
// If follow is true, the method subscribes to live log streaming after
// the initial GET completes and continues writing new log entries until
// the provided context is canceled.
func (ac *AdamClient) WriteDeviceLogs(ctx context.Context, devUUID uuid.UUID,
	match logger.LogEntryMatcher, logWriter logger.DeviceLogWriter, follow bool) error {
	if err := ac.checkAdamRunning(); err != nil {
		return err
	}
...
// WriteDeviceInfoMsgs retrieves informational messages (ZInfoMsg)
// published by the specified device and passes matching messages to writer.
//
...
// are written. If match is nil, all messages are written.
//
// The provided writer is not closed by this function.
func (ac *AdamClient) WriteDeviceInfoMsgs(ctx context.Context, devUUID uuid.UUID,
	match func(msg *eveinfo.ZInfoMsg) bool, writer InfoMsgWriter, follow bool) error {
	if err := ac.checkAdamRunning(); err != nil {
		return err
	}
	...
	for {
		...
		if match == nil || match(msg) {
			if err = writer.Write(msg); err != nil {
				return fmt.Errorf("failed to write info message: %w", err)
			}
		}
	}
...
func (ac *AdamClient) SubscribeToDeviceInfoMsgs(devUUID uuid.UUID,
	...
	go func() {
		defer wg.Done()
		ac.streamDeviceInfoMsgs(streamCtx, devUUID, match, channel)
	}()
...
// streamDeviceInfoMsgs establishes a streaming connection to receive
// informational messages from the specified device.
//
// It reconnects on transient failures, applies the optional match filter,
// and forwards matching messages to infoMsgCh until ctx is canceled.
// The channel is closed before the function returns.
func (ac *AdamClient) streamDeviceInfoMsgs(ctx context.Context, devUUID uuid.UUID,
	match func(msg *eveinfo.ZInfoMsg) bool,
	infoMsgCh chan<- *eveinfo.ZInfoMsg) {
	const retryDelay = 3 * time.Second
	defer close(infoMsgCh)
	...
	for {
		...
		func() {
			...
			for {
				...
				if err = protojson.Unmarshal(raw, msg); err != nil {
					ac.log.Errorf(
						"failed to proto-unmarshal streamed info message: %v",
						err,
					)
					continue
				}
