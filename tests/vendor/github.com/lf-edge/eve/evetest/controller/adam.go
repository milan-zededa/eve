...
//
// Request events are delivered asynchronously to the provided channel.
//
...
func (ac *AdamClient) SubscribeToDeviceRequests(
	...
	go func() {
		defer wg.Done()
		ac.streamAPIRequests(streamCtx, devUUID, channel)
	}()
...
func (ac *AdamClient) SubscribeToAppLogs(
	...
	go func() {
		defer wg.Done()
		ac.streamAppLogs(streamCtx, devUUID, appUUID, match, channel)
	}()
...
// streamAppLogs maintains a long-lived streaming connection to Adam in
// order to observe log entries originating from the specified application
// instance.
//
// The function automatically reconnects after transient failures using a
// fixed retry delay. Each received log record is decoded and, if it matches
...
//
// This function is intended to be executed in its own goroutine.
func (ac *AdamClient) streamAppLogs(
	ctx context.Context, devUUID, appUUID uuid.UUID, match logger.LogEntryMatcher,
	logCh chan<- *evelogs.LogEntry) {
	const retryDelay = 3 * time.Second
	defer close(logCh)
	...
	for {
		...
		func() {
			...
			for {
				...
				if err = protojson.Unmarshal(raw, entry); err != nil {
					ac.log.Errorf(
						"failed to proto-unmarshal streamed app log entry: %v",
						err,
					)
					continue
				}
...
func (ac *AdamClient) SubscribeToDeviceMetrics(devUUID uuid.UUID,
	...
	go func() {
		defer wg.Done()
		ac.streamDeviceMetrics(streamCtx, devUUID, channel)
	}()
...
// streamDeviceMetrics establishes a streaming connection to receive
// metric messages from the specified device.
//
// It reconnects on transient failures and forwards messages to metricMsgCh
// until ctx is canceled.
// The channel is closed before the function returns.
func (ac *AdamClient) streamDeviceMetrics(ctx context.Context, devUUID uuid.UUID,
	metricMsgCh chan<- *evemetrics.ZMetricMsg) {
	const retryDelay = 3 * time.Second
	defer close(metricMsgCh)
	...
	for {
		...
		func() {
			...
			for {
				...
				if err = protojson.Unmarshal(raw, msg); err != nil {
					ac.log.Errorf(
						"failed to proto-unmarshal streamed metric message: %v",
						err,
					)
					continue
				}
