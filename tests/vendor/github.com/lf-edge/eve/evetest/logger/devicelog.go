...
// EVE device logs. Implementations may write to a file,
// stream over gRPC, or perform filtering/formatting.
type DeviceLogWriter interface {
	Write(*evelogs.LogEntry) error
}
...
// WriteEntry formats the given LogEntry as a human-readable line
// and writes it to the underlying OutFile.
func (w *PlainDeviceLogFile) Write(entry *evelogs.LogEntry) error {
	var ts string
...
// Write formats the given LogEntry as an api.LogMessage and sends
// it over the gRPC stream. Severity is converted from EVE's string format
// to the corresponding api.LogSeverity.
func (w *GrpcDeviceLogStreamer) Write(entry *evelogs.LogEntry) error {
