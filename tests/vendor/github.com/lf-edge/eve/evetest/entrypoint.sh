...
if [ -z "$EVETEST_NAME" ]; then
    echo "Error: EVETEST_NAME is not set"
    exit 1
fi
...
go test $GO_TEST_FLAGS -timeout 0 -parallel 1 -p 1 \
        -run "^${EVETEST_NAME}$" ./... | tee "$GO_TEST_OUTPUT_FILE" &
GO_TEST_PID=$!
...
cleanup() {
    ...
    [ -n "$GO_TEST_PID" ] && wait_with_timeout "go-test" "$GO_TEST_PID" 60
    set_artifact_ownership

    # Wait for broker to exit
    [ -n "$BROKER_PID" ] && wait_with_timeout "broker" "$BROKER_PID" 60
...
if [ -n "$BROKER_PID" ]; then
    ...
    wait_with_timeout "broker" "$BROKER_PID" 60 2>/dev/null
fi
