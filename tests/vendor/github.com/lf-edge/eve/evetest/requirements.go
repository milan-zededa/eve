...
const (
	...
	RebootEdgeDevice
	// ResetDeviceConfig : resets device configuration to what is specified in
	// RequireEdgeDevice.WithInitialDeviceConfig.
	ResetDeviceConfig
...
// RequireEdgeDeviceCluster : requirement to deploy cluster of EVE devices.
type RequireEdgeDeviceCluster struct {
	// Specify requirements for individual cluster devices or leave empty
	// if test does not have any specific requirements for individual devices.
	EdgeDevices []RequireEdgeDevice

	// If an existing cluster matches requirements, this can be enabled to recreate
	// the K3s clustering before the test starts.
	RecreateK3sCluster bool
}

func (r RequireEdgeDeviceCluster) isRequirement() {}
