# custom-resolver.md — DNS Resolution Beyond the 3-Server resolv.conf Limit
...
NIM continues to write `/etc/resolv.conf` with all DNS servers interleaved across
interfaces. This serves non-Go binaries — shell tools, diagnostics, anything a user runs
over SSH — which read the file directly. The 3-nameserver limitation is accepted for that
audience: it affects interactive tooling, not automated management traffic.
The goal of this change is solely to ensure that **EVE management traffic**
(controller connectivity, EVE/app image download, port probing, diagnostics) is no
longer subject to that limit.
...
instance forwards DNS queries to the upstream DNS servers of the NI's uplink port. Apps
...
listening on a loopback address (e.g. `127.0.0.2:53`). Configure it with all mgmt port
...
`/etc/resolv.conf` lists `127.0.0.2` as the sole nameserver. All Go resolvers, nettrace,
third-party libraries, and SSH tools pick this up without any code changes.
...
**portprober for NI-attached (app-shared) ports**: the portprober should use the
NI's existing per-NI dnsmasq instance rather than the mgmt dnsmasq, since the NI
already has the correct upstream servers for its uplink port. This path needs no
change.

### Search domains

dnsmasq supports `domain-search` directives, so DHCP-provided search domains can be
propagated the same way as upstream servers. NIM writes them when regenerating the
mgmt dnsmasq config.

### Code changes with Option B

Compared to Option A, the `pillar/dialer` package is significantly reduced:
- **No custom resolver** needed anywhere — `net.DefaultResolver` reads
  `/etc/resolv.conf` → mgmt dnsmasq. DNS queries go to `127.0.0.x` (loopback), so
  source IP binding on the resolver socket is irrelevant and must not be set.
  `ResolverWithLocalIP`, `dnsResolver` (portprober), and the inline resolver in `diag`
  are all deleted.
- **No custom dialer** needed — `net.Dialer{LocalAddr: ...}` is sufficient for TCP.
  `LocalAddr` is set only for the TCP connection to the external endpoint (source-based
  routing), not for DNS. `DialerWithResolverCache` is simplified to cache-lookup +
  plain `net.Dialer`.
...
- `nettrace`: **no API changes**. `SourceIP` in `HTTPClientCfg` is still set by the
  ...
  `/etc/resolv.conf` → mgmt dnsmasq transparently. The DNS query socket binding to
  `SourceIP` that nettrace performs is harmless (dnsmasq on loopback will answer
  regardless of source), so no code change is required.
...
| Search domains | Fixed (dnsmasq domain-search) | Fixed (Resolver.SearchDomains) |
...
| portprober (NI-attached ports) | Uses existing NI dnsmasq — no change | Uses Resolver |
| Code to write and maintain | Small (NIM dnsmasq config gen) + minimal Resolver residue | Full pillar/dialer package |
| Third-party / SSH tools | All benefit automatically | No change (resolv.conf unchanged) |
...
Option B is a serious contender. Its primary advantage is substantially less custom Go
code to maintain: all call sites reduce to `net.Dialer{LocalAddr: ...}` with no
resolver customisation at all — `pillar/dialer` is not needed. The infrastructure
already exists (per-NI dnsmasq) and NIM already manages similar configs. DPC
verification uses dnsmasq just like real traffic does, which is the correct behaviour.

The choice between A and B is primarily a maintenance trade-off: Option A gives a
self-contained, fully testable package with perfect per-port isolation; Option B keeps
the codebase smaller at the cost of an operational dependency and slightly weaker
per-port guarantees in the verification path.

---

## Recommendation

Both options are viable. The choice is a maintenance trade-off:

**Option A** (`pkg/pillar/dialer`) if the team prefers a fully self-contained,
...
library (`nettrace`, `zedUpload`) needs any API change or vendored patch; the dnsmasq
infrastructure already exists in EVE; and verification correctly mirrors real traffic
...
`pkg/pillar/dialer` replicates it faithfully.
