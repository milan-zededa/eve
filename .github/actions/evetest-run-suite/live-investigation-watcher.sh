...
while true; do
    ...
    case "$RESP" in
        *"Test failed with:"*)
            ...
            # The failure message itself is often multi-line (e.g. Gomega's
            # "Timed out after ...\nExpected\n...\nto satisfy...", with an
            ...
            if [ -z "$ARTIFACT_DIR" ]; then
                ...
                # Only set on a PR run (e.g. via an "/evetest live-analysis"
                # comment) -- lets Claude judge whether this failure is
                # plausibly caused by the PR, same as the post-mortem path.
                PR_NOTE=""
                if [ -n "${PR_NUMBER:-}" ]; then
...
"Bash(cat:*),Bash(head:*),Bash(tail:*),Bash(find:*),Bash(jq:*),Bash(wc:*),Bash(sort:*),Bash(uniq:*),Read,Grep,Glob" \
                ...
                {
                    # Each block is normal standalone markdown in its own
                    # right, starting at "#" -- action.yml's "Build
                    # claude-analysis.md" step demotes it (via
                    # shift-headings.sh) to nest under claude-analysis.md's
                    # own title, so this script doesn't need to know or
                    # guess that final depth.
                    echo "# ${CURRENT_TEST}"
                    echo
                    echo "## Failure"
                    echo
